...
<table x:str border="0" cellpadding=0 cellspacing=0 width=716 style='border-collapse: collapse;table-layout:fixed;width:536pt' u1:str>
 ...
 <tr height=29 style='mso-height-source:userset;height:21.75pt'>
  ...
  <td colspan=2 class=xl33 width=238 style='border-right:.5pt solid black;  border-left:none;width:178pt'>당기<br><font style="size:8pt">(<%=StringUtil.substr(obox.get("acc_start_ymd"),0,4)%>년  <%=StringUtil.substr(obox.get("acc_start_ymd"),4,6)%>월 <%=StringUtil.substr(obox.get("acc_start_ymd"),6)%>일 부터 &nbsp;<%=box.get("std_year")%>년  <%=box.get("std_month")%>월  <%=DateUtil.getDayCount(box.getInt("std_year"), box.getInt("std_month"))%>일)</font></td>
  <td colspan=2 class=xl35 width=238 style='border-right:.5pt solid black;  border-left:none;width:178pt'>전기<br><font style="size:8pt">(<%=Integer.parseInt(StringUtil.substr(obox.get("acc_start_ymd"),0,4))-1%>년  <%=StringUtil.substr(obox.get("acc_start_ymd"),4,6)%>월 <%=StringUtil.substr(obox.get("acc_start_ymd"),6)%>일 부터 &nbsp;<%=Integer.parseInt(StringUtil.substr(obox.get("acc_end_ymd"),0,4))-1%>년  <%=StringUtil.substr(obox.get("acc_end_ymd"),4,6)%>월 <%=StringUtil.substr(obox.get("acc_end_ymd"),6)%>일)</font></td>
 </tr>
