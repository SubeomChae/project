...
<input type="hidden" name="file_path"	value="">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr height="40">			
			<th width="15%">(1)사업자등록번호</th>
			<td width="35%"><%=FormatUtil.bizRegiNoFormat(obox.get("saupja_no"))%></td>
			<th width="15%">(2)상호(법인명)</th>
			<td width="35%"><%=obox.get("corp_nm")%></td>
		</tr>
		<tr height="40">
			<th>(3)성명(대표자)</th>
			<td style="padding:0 5 0 5"><%=obox.get("rep_person_nm")%></td>
			<th>(4)사업장 소재지</th>
			<td style="padding:0 5 0 5"><%=obox.get("addr")%></td>
		</tr>
		<tr height="40">
			<th>(5)거래기간</th>
			<td style="padding:0 5 0 5"><%=obox.get("tax_ymd_from")%>~<%=obox.get("tax_ymd_to")%></td>
			<th>(6)작성일자</th>
			<td style="padding:0 5 0 5"><%=obox.get("tax_ymd_to")%></td>
		</tr>
	</table>
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr height="28">
			<th width="30%">구분</th>
			<th width="15%">(7) 매입처수</th>
			<th width="15%">(8) 매수</th>
			<th width="40%">(9) 매  입  금  액</th>
		</tr>
		<tr height="40">
			<th>합계</th>
			<td class="t-center"><%=vbox.get("cust_cnt")%></td>
			<td class="t-center"><%=vbox.get("cust_slip_cnt")%></td>
			<td class="t-center"><%=FormatUtil.insertComma(String.valueOf(vbox.get("cust_gong_amt")))%></td>
		</tr>
		<tr height="40">
			<th>과세기간 종료일 다음 달 11일까지 전송된 <br>전자 세금계산서 발급받은 분</th>
			<td class="t-center"><%=vbox.get("cust_cnt_y")%></td>
			<td class="t-center"><%=vbox.get("cust_slip_cnt_y")%></td>
			<td class="t-center" style="padding-right:5px;"><%=FormatUtil.insertComma(String.valueOf(vbox.get("cust_gong_amt_y")))%></td>
		</tr>
		<tr height="40">
			<th>위 전자 세금계산서 외의 발급받은분</th>
			<td class="t-center"><%=vbox.get("cust_cnt_n")%></td>
			<td class="t-center"><%=vbox.get("cust_slip_cnt_n")%></td>
			<td class="t-center"><%=FormatUtil.insertComma(String.valueOf(vbox.get("cust_gong_amt_n")))%></td>
		</tr>
	</table>
	
	<table class="table table-view table-bordered table-cover marB5">
		<tr height="28">
			<th width="10%">(10) 번호</th>
			<th width="15%">(11) 사업자등록번호</th>
			<th width="15%">(12) 상호(법인명)</th>
			<th width="10%">(13) 매수</th>
			<th width="40%">(14) 매 입 금 액</th>
			<th width="10%">비고</th>
		</tr>
	<% if(rsWp.getLength() == 0) { %>
		<tr height="28"><td colspan="10" class="t-center">검색된 자료가 없습니다.</td></tr>
	<% }else{ 
		for(int i=0; i < rsWp.getLength(); i++) { %>
		<tr height="28">
			<td class="t-center"><%=rsWp.get("rn",i)%></td>
			<td class="t-center"><%=rsWp.get("saupja_no_x",i)%></td>
			<td class="t-center" style="letter-spacing:-1px"><%=rsWp.get("cust_nm",i)%></td>
			<td class="t-center"><%=rsWp.get("slip_cnt",i)%></td>
			<td class="t-center" style="padding-right:5px;"><%=rsWp.get("gong_amt",i,Box.THOUSAND_COMMA)%></td>
