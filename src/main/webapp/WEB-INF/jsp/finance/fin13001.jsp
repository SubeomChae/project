...
<script language="javascript">
	function goSearch(slip_no) {
		...
		ajaxReplaceForm("fin13001_list.do", "#main", "#divListData", function() {
			if(slip_no == '') { 
				goWrite();
			}else {
				goDetail(slip_no); 
	...
	function goWrite(){
		...
		ajaxReplaceForm("fin13002_write.do?method=write", "#main", "#divFrame");
	}
	
	function goDetail(slip_no) {
		$("tr[id^='tr_']").removeClass("bg-yellow");
		if(slip_no != '') { $("tr[slip_no='"+slip_no+"']").addClass("bg-yellow"); }
		var pageUrl = 'fin13002_edit.do?method=edit&slip_no='+slip_no;
				...
				<td>
					<div class="form-inline">
						<div class="input-group input-medium date date-picker" data-date="<%=box.get("slip_ymd_from",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
							<input type="text" name="slip_ymd_from" value="<%=box.get("slip_ymd_from")%>" maxlength="8" title="작성일자(부터)" class="form-control tYMD">
						...
						<div class="input-group input-medium date date-picker" data-date="<%=box.get("slip_ymd_to",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
							<input type="text" name="slip_ymd_to" value="<%=box.get("slip_ymd_to")%>" maxlength="8" title="작성일자(까지)" class="form-control tYMD">
							<span class="input-group-btn"><button class="btn dark" type="button"><i class="fa fa-calendar"></i></button></span>
						</div>
					</div>		
				</td>
...
<% } %>
	<div class="row row-set">
		<div class="col-md-4 col-xs-4 col-sm-4 row-set-col padL0 padR0">
			<div id="divArea" class="table-scrollable scrollOptionY" style="height:calc(100vh - 155px)">
