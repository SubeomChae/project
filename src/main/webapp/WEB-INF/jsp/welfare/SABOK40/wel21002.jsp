...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
		...
		<% }else {
					...
					<%if(!box.get("excel_yn").equals("Y")){%>
						<%if(!bugt_cd.equals("S700") && !bugt_cd.equals("S008")){%>
							<%if(rsWp.get("biz_type",i).equals("A")){%>
								<a href="javascript:go_slip_viewer('<%=rsWp.get("slip_no",i)%>');">
							<%}else{ %>
								<%if(rsWp.get("bugt_nm",i).equals("복지카드")){%>
									<a href="javascript:go_employee_info('<%=rsWp.get("empl_no",i)%>')">
								<%}else{%>
									<a href="javascript:go_welfare_view('<%=rsWp.get("apply_no",i)%>','<%=rsWp.get("bugt_cd",i)%>');">
								<%}%>
							<%} %>
						<%}else{%>
							<a href="javascript:go_employee_info('<%=rsWp.get("empl_no",i)%>')">
				...
				<% } %>
				<% if(bugt_cd.equals("")) { %>
					<td width="10%" align="center"><%=rsWp.get("biz_type",i).equals("A")?rsWp.get("subject",i):rsWp.get("etc_remark",i)%></td>
					<td width="10%" align="center"><a href="javascript:go_slip_viewer('<%=rsWp.get("slip_no",i)%>');"><%=rsWp.get("slip_no",i,Box.SLIP_NO)%></a></td>
				<% } %>
