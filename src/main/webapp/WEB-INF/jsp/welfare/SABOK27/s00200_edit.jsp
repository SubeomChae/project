...
<script language="javascript">
	...
	function CalendarSettingPost(target) {
		...
		document.saveForm.inner_ilsu.value = inner_ilsu;
		formatMaskElm(document.saveForm.inner_ilsu);
	}
</script>
...
<%@ include file="/WEB-INF/jsp/welfare/payGuide.jsp" %>
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			...
			<td width="35%">
			<% if(uSesEnt.dbo.equals("SABOK26")) { %>
				<select name="family_rel_cd" title="가족관계" class="bs-select form-control required" style="width:80px">
		...
		<% } %>
			<th>사용금액</th>
			<td>
				<input type="text" name="use_amt" value="<%=obox.get("use_amt")%>" size="8" maxlength="10" title="사용금액" class="form-control tMONEY" style="width:70px" >&nbsp;원
			</td>
		</tr>
		<tr>
			...
			<td colspan="3">
				<div class="form-inline">
					<div class="input-group input-medium date date-picker" data-date="<%=obox.get("inner_start_ymd",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" name="start_ymd" value="<%=obox.get("inner_start_ymd")%>" maxlength="8" title="입원기간(부터)" class="form-control tYMD" onchange="CalendarSettingPost('start_ymd')">
						<span class="input-group-btn"><button class="btn dark" type="button"><i class="fa fa-calendar"></i></button></span>
					</div>
					<span>&nbsp;부터&nbsp;~&nbsp;</span>
					<div class="input-group input-medium date date-picker" data-date="<%=obox.get("inner_end_ymd",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" name="end_ymd" value="<%=obox.get("inner_end_ymd")%>" maxlength="8" title="입원기간(까지)" class="form-control tYMD" onchange="CalendarSettingPost('end_ymd')">
						<span class="input-group-btn"><button class="btn dark" type="button"><i class="fa fa-calendar"></i></button></span>
					</div>&nbsp;까지&nbsp;
		...
		<tr>
			<th>계좌번호</th>
			<td colspan="3">
				<select name="bank_cd" title="금융기관" <%if(!uSesEnt.corp_cd.equals("SABOK27")){%>class="bs-select form-control required"<%}%> class="bs-select form-control required" style="width:120px">
					<option value=''>:: 금융기관 선택 ::</option>
		            <%=CodeUtil.optList(codeWp, "bank_cd", obox.get("bank_cd"))%>
				</select>
				<input type="text" name="account_no" value="<%=obox.get("account_no")%>" size="18" maxlength="30" title="계좌번호" <%if(!uSesEnt.corp_cd.equals("SABOK27")){%>class="form-control required"<%}%> class="form-control required" style="width:100px">&nbsp;예금주 :
				<input type="text" name="account_owner" value="<%=obox.get("account_owner")%>" size="10" maxlength="15" title="예금주" <%if(!uSesEnt.corp_cd.equals("SABOK27")){%>class="form-control required"<%}%> class="form-control required" style="width:100px">
			</td>
		</tr>
		<tr>
			<th>비고</th>
