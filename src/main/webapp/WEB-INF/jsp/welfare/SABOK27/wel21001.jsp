...
<script language='javascript'>
	...
	function changeBugtCd(bugt_cd) {
		...
		}else if(bugt_cd == 'S701') {
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='bs-sslect form-control required' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeUtil.optList("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S701","","","",uSesEnt.dbo)%></select> ";
			document.main.excel_file_nm.value = '연금지원현황';		
		...
		if(bugt_cd == 'S700') { 
			...
			document.getElementById('divApplyYmd').innerHTML = '지원일자';
			document.getElementById('divText').innerHTML = '세부정보를';
		}else { 
		...
		<table id="MyTable" class="table table-bordered table-hover order-column">
			...
			<% }else {
				...
				<% } %>
					...
					<td width="7%" class="t-center"><%if(!box.get("excel_yn").equals("Y")){%><%if(!box.get("bugt_cd").equals("S007") && !box.get("bugt_cd").equals("S008")){%><a href="javascript:goWelfareView('<%=rsWp.get("apply_no",i)%>','<%=rsWp.get("bugt_cd",i)%>');"><%}else{%><a href="javascript:go_employee_info('<%=rsWp.get("empl_no",i)%>')"><%}%><%}%><%=rsWp.get("empl_nm",i)%></a></td>
