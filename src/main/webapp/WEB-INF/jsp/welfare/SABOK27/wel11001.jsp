...
<script language='javascript'>
	function goSearch(gbn) {
		if(formValidate("#main")) {		
			...
			removeMask("#main");	
			document.main.unit_count.value = 500; 
	...
	function goSave() {
		if(formValidate("#main")) {
			...
			if(confirm('연금지원 대상자를 일괄 저장하시겠습니까?')) {
				removeMask("#main");		
	...
	function goDelete() {
		if(formValidate("#main")) {
			if(confirm('연금지원 대상자를 일괄 삭제하시겠습니까?')) {
				removeMask("#main");				
	...
	function concatPayCond() {
		var pay_cond = document.form2.entry_ymd1.value.replace(/-/g,"") + "|" + document.form2.entry_ymd2.value.replace(/-/g,"") + "|";
		for(var i=0; i < document.form2.empl_class.length; i++) {
			if(document.form2.empl_class[i].checked) { pay_cond += document.form2.empl_class[i].value; }
		}	
		pay_cond += "|";			  
		for(var i=0; i < document.form2.empl_status.length; i++) {
			if(document.form2.empl_status[i].checked) { pay_cond += document.form2.empl_status[i].value; }
		}	
...
<input type="hidden" name="discount_amt" 	value="<%=obox.getInt("discount_amt")%>">
<input type="hidden" name="member_cnt" 		value="<%=obox.getInt("member_cnt")%>">
<input type="hidden" name="sort_order" 		value="">
<input type="hidden" name="unit_count" 		value="">
<input type="hidden" name="page_count"		value="">
<input type="hidden" name="excel_file_nm" 	value="연금지원">
<% if(!box.get("msg_popup").equals("Y")) { %>
	<h1 class="page-title">연금지원</h1>
<% } %>
			...
			<div class="form-inline f-right">
				<div class="form-group">
			  	...
			  	<% }else if(isEditable) { %>
					...
					- (할인액 : <input type="text" name="discount_amt" value="<%=obox.getInt("discount_amt")%>" title="할인액" class="form-control acc-border tMONEY" style="width:100px" onChange="document.form1.pay_amt_tot.value = parseInt(document.form1.member_cnt.value*<%=pbox.getLong("unit_amt")%>) - parseInt(this.value); formatMaskElm(document.form1.pay_amt_tot);"> 원) 
					= <input type="text" name="pay_amt_tot" value="<%=String.valueOf(obox.getInt("pay_amt") - obox.getInt("discount_amt"))%>" title="합계" class="form-control acc-border tMONEY" style="width:100px" readOnly> 원
			  	<% } %>
			  	</div>
			</div>
		</div>
       	<div id="divArea" class="table-scrollable scrollOptionY" style="height:calc(100vh - 195px)">
       	<% } %>
			<table id="MyTable" class="table table-bordered table-hover order-column">
				...
				<thead>
					<tr>
						<th>No</th>
						<th><%if(isEditable){%><a href="javascript:goSort('t1.empl_no')">사번</a><%}else{%><a href="javascript:goSort('t1.empl_no')">사번</a><%}%></th>
						<th><%if(isEditable){%><a href="javascript:goSort('t1.empl_nm')">성명</a><%}else{%><a href="javascript:goSort('t2.empl_nm')">성명</a><%}%></th>
						<th><%if(isEditable){%><a href="javascript:goSort('t2.dept_nm')">부서</a><%}else{%><a href="javascript:goSort('dept_nm')">부서</a><%}%></th>
						<th><%if(isEditable){%><a href="javascript:goSort('t1.position_cd')">직위</a><%}else{%><a href="javascript:goSort('t1.position_cd')">직위</a><%}%></th>
						<th><%if(isEditable){%><a href="javascript:goSort('t1.entry_ymd')">입사일자</a><%}else{%><a href="javascript:goSort('t2.entry_ymd')">입사일자</a><%}%></th>
						<th><%if(isEditable){%><a href="javascript:goSort('t1.reside_no')">주민번호</a><%}else{%><a href="javascript:goSort('reside_no')">주민번호</a><%}%></th>
						<th><%if(isEditable){%><a href="javascript:goSort('t1.empl_class')">사원구분</a><%}else{%><a href="javascript:goSort('empl_class_nm')">사원구분</a><%}%></th>
						<th><%if(isEditable){%><a href="javascript:goSort('t1.empl_status')">재직상태</a><%}else{%><a href="javascript:goSort('empl_status_nm')">재직상태</a><%}%></th>
						<th>지급액</th>
					<%if(isEditable && !box.get("excel_yn").equals("Y")){%>
						<th>
							<label class="mt-checkbox mt-checkbox-outline">
								<input type="checkbox" id="checkAll" name="checkAll" value="Y" style="border:none" onClick="clickCheckAll(this.checked)"><span/>
							</label>
						</th>
					<% } %>
	          		</tr>
				...
				<% if(!box.get("bugt_cd").equals("") && rsWp.getLength() == 0) { %>
						<tr height="25"><td colspan="<%=isEditable ? 11 : 10%>" class="t-center">등록된 자료가 없습니다.</td></tr>
				<% }else if(rsWp.getLength() > 0) {
					   String old_empl_no = rsWp.get("sort_key",0) + "@" + rsWp.get("sort_key",rsWp.getLength()-1);
						...
						<%if(isEditable && !box.get("excel_yn").equals("Y")){%>
							<td class="t-center">
								<label class="mt-checkbox mt-checkbox-outline">
									<input type="checkbox" name="new_empl_no<%=i%>" value="<%=rsWp.get("empl_no",i)%>," style="border:none" <%=rsWp.get("exist_yn",i).equals("Y")?"checked":""%> onClick="autoCalc('<%=i%>')"><span/>
								</label>
							</td>
						<% } %>
						</tr>	
...
<!-- 대상자추출 -->	
	...
	<form name="form2" method="post" action="" class="fType" style="margin:0px">
...
<input type="hidden" name="subject" 		value="연금지원(<%=obox.get("bugt_nm")%>)">
