...
<script language='javascript'>
	...
	%>	
		if(inx == '') {
			document.form1.member_cnt.value = document.form1.checkAll.checked ? <%=cnt+rsWp.getLength()%> : <%=cnt%>;
		}else {
			...
			<tr>
				...
				<% }else { %>
					...
					<% if(obox.get("sign_status").equals("1")) { %>
						...
						<button type="button" class="btn btn-primary btn-table" onClick="goDeleteSlip();">전체삭제</button>
					...
					<%if(isEditable){%>
						<th>
							<label class="mt-checkbox mt-checkbox-outline">
								<input type="checkbox" name="checkAll" value="Y" onClick="clickCheckAll(this.checked);"><span/>
							</label>
						</th>
					<%}%>
	          		</tr>
				...
				<% }else if(rsWp.getLength() > 0) {
				   String old_empl_no = rsWp.get("sort_key",0) + "@" + rsWp.get("sort_key",rsWp.getLength()-1);
					...
					<tr> 
						...
						<td class="t-center"><%if(!box.get("excel_yn").equals("Y")){%><a href="javascript:goEmployeePop('&empl_no=<%=rsWp.get("empl_no",i)%>')"><%}%><%=rsWp.get("empl_nm",i)%></a></td>
