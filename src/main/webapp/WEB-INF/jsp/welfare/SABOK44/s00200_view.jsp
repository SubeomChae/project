...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<div class="table-scrollable">
		<table class="table table-view table-bordered">
			<tr>
				...
				<th width="15%">질환명</th>
				<td width="35%"><%=obox.get("disease_nm")%></td>
			</tr>
			<tr>
				<th>의료기관</th>
				<td><%=obox.get("medi_org_nm")%></td>
			<% if(!uSesEnt.dbo.equals("SABOK26")) { %>
				<th>소재지</th>
				<td ><%=obox.get("medi_org_addr")%></td>
			</tr>
			<tr>
				<th>진료일자</th>
				<td><%=obox.get("medi_ymd",Box.DEF_DATE_FMT)%></td>
			<% } %>
				<th>사용금액</th>
	...
	<div class="table-bottom-control">
		...
		<% } %>
			<button type="button" class="btn dark" onClick="goUbiViewer('welfare/<%=box.get("dbo")%>/report/s002.jrf', 'P_PARAM01!SLP002,<%=box.get("apply_no")%>!P_PARAM02!ACCTB_SLIP01|<%=obox.get("slip_no")%>|<%=obox.get("apply_no")%>|<%=obox.get("bugt_cd")%>,<%=obox.get("bugt_cd")%>!');"><img src="/com/img/ico_btn_print.png">인쇄</button>
		</span>
	</div>
</form>
