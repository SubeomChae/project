...
<%	String bank_cd;
	...
	RsWrapper codeWp = CodeHelper.getRsWrapper("PR_COMTB_CODE02_LIST","MULTCODE", "", "school_flag"+bank_cd+"family_rel_cd|","","","","","",request);
	boolean isEditable = false;
	if(obox.get("sign_status").equals("") || obox.get("sign_status").equals("1") || obox.get("sign_status").equals("4")) { isEditable = true; }
%>

<title>학자금 신청서 작성</title>
	...
	function goSave() {
		if (formValidate("#main")) {
			if(confirm('저장하시겠습니까?')) {
				...
				document.main.data04.value = document.main.school_nm.value;
				document.main.data05.value = document.main.school_flag.value;
				document.main.data06.value = document.main.major_nm.value;
				document.main.data07.value = document.main.school_year.value;
				document.main.data08.value = document.main.school_term.value;
			<% if(uSesEnt.dbo.equals("SABOK26")) { %>
				document.main.data09.value = document.main.grade_score.value;
				document.main.data10.value = document.main.pay_amt.value;
			<% } %>
...
<table width="100%" border="0" cellspacing="0" bgcolor="#ffffff">
	...
	<tr>
		<td valign="top">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table01">
				<tr height="28">
					<td align="center" class="table01_title">학교명</td>
					<td colspan="3" style="padding-left:5px">
						<input type="text" name="school_nm" value="<%=obox.get("school_nm")%>" size="10" maxlength="30" title="학교명" class="required" style="border:1 solid gray">
					<% if(uSesEnt.dbo.equals("SABOK26")) { %>
						대학교<input type="hidden" name="school_flag" value="5">
					<% }else { %>
						<select name="school_flag" title="학교" class="required" style="width:100px">
							<option value=''>:: 학교선택 ::</option>
				            <%=CodeHelper.OptList(codeWp, "school_flag", obox.get("school_flag"))%>
						</select>
				    <% } %>
						&nbsp;<input type="text" name="school_year" value="<%=obox.get("school_year")%>" size="2" maxlength="1" title="학년" class="required" style="border:1 solid gray"> 학년					
						&nbsp;&nbsp;<input type="text" name="school_term" value="<%=obox.get("school_term")%>" size="2" maxlength="1" title="학기" style="border:1 solid gray"> 학기													
					</td>
				</tr>	
				<tr height="28">	
					<td width="15%" align="center" class="table01_title">학과</td>
					<td width="35%" style="padding-left:5px"><input type="text" name="major_nm" value="<%=obox.get("major_nm")%>" size="20" maxlength="20" title="학과" style="border:1 solid gray"></td>
					<td width="15%" align="center" class="table01_title">대상자</td>
					<td width="35%" style="padding-left:5px">
					<% if(uSesEnt.dbo.equals("SABOK26")) { %>
						<select name="family_rel_cd" class="required" title="가족관계" style="width:70px;">
			...
			<% if(uSesEnt.dbo.equals("SABOK26")) { %>
				<tr height="28">
					<td align="center" class="table01_title">학점</td>
					<td style="padding-left:5px"><input type="text" name="grade_score" value="<%=obox.get("grade_score")%>" size="13" maxlength="10" class="required" title="학점" style="width:90px;border:1 solid gray" ></td>
					<td align="center" class="table01_title">납부금액</td>
					<td style="padding-left:5px"><input type="text" name="pay_amt" value="<%=obox.get("pay_amt")%>" size="13" maxlength="10" class="required tMONEY" title="납부금액" style="width:90px;border:1 solid gray" > 원</td>
				</tr>
			<% } %>
				<tr height="28">
				...
				<tr height="28">
					...
					<td colspan="3" style="padding-left:5px">
						<select name="bank_cd" title="금융기관" <%if(!uSesEnt.corp_cd.equals("SABOK27")){%>class="required"<%}%> style="width:120px">
