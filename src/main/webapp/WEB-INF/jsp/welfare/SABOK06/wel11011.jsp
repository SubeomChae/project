...
<script language='javascript'>
	...
	function goCreateSlip() {
		if(formValidate("#main")) {
			if(confirm('전표를 생성하시겠습니까?')) {
				removeMask("#main");				
				document.main.action = 'wel11011_create_slip.do';
				document.main.target = '_self';
				document.main.submit();
			}
		}
	...
	function goDelete() {
		if(formValidate("#main")) {
			if(confirm('생일펀드지급 대상자를 일괄 삭제하시겠습니까?')) {
				removeMask("#main");				
				document.main.action = 'wel11011_delete.do';
				document.main.target = '_self';
				document.main.submit();
			}
		}
...
<input type="hidden" name="excel_file_nm" 	value="생일펀드일괄지급">
<% if(!box.get("msg_popup").equals("Y")) { %>
	<h1 class="page-title">생일펀드 일괄지급</h1>
<% } %>
		...
		<div class="modal-dialog" style="width:400px;">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">생일펀드지급 대상자 추출 조건설정</h4>
				</div>
...
<input type="hidden" name="subject" 		value="생일펀드지급(<%=obox.get("bugt_nm")%>)">
