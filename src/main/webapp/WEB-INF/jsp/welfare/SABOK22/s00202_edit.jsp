...
<%@ include file="/WEB-INF/jsp/welfare/payGuide.jsp" %>
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>병명</th>
			<td colspan="3">
				<input type="text" name="disease_nm" value="<%=obox.get("disease_nm")%>" maxlength="50" title="병명" class="form-control"  style="width:250px;">
			</td>
		</tr>
		<tr>
			<th>치료기간</th>
			<td colspan="3">
				<div class="form-inline">
					<div class="input-group input-medium date date-picker" data-date="<%=obox.get("inner_start_ymd",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" name="start_ymd" value="<%=obox.get("inner_start_ymd")%>" maxlength="8" title="치료기간(부터)" class="form-control tYMD" onChange="CalendarSettingPost('start_ymd')">
					...
					<div class="input-group input-medium date date-picker" data-date="<%=obox.get("inner_end_ymd",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" name="end_ymd" value="<%=obox.get("inner_end_ymd")%>" maxlength="8" title="치료기간(까지)" class="form-control tYMD" onChange="CalendarSettingPost('end_ymd')">
						<span class="input-group-btn"><button class="btn dark" type="button"><i class="fa fa-calendar"></i></button></span>
					</div>&nbsp;까지&nbsp;
					(총&nbsp;<input type="text" name="inner_ilsu" value="<%=obox.get("inner_ilsu")%>" maxlength="3" title="입원일수" class="form-control tMONEY" style="width:30px" readOnly>&nbsp;일)
				</div>
			</td>
		</tr>
		<tr>
