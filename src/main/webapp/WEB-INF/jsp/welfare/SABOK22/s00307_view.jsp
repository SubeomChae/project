...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("학자금 신청서 조회(안양)");	
		<% } %>
	});
</script>
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>학교명</th>
			<td colspan="3"><%=obox.get("school_nm")%><%=obox.get("school_flag_nm")%> <%=obox.get("school_year")%>학년 <%=obox.get("school_term")%><%if(!obox.get("school_term").equals("")){%>학기<%}%></td>
		</tr>
		<tr>
			<th width="15%">전공</th>
			<td width="35%"><%=obox.get("major_nm")%></td>
			<th width="15%">자녀명</th>
			<td width="35%"><%=obox.get("family_nm")%></td>					
		</tr>
		<tr>
			<th>신청일자</th>
		...
		<tr>
  		...
  		<tr>
			...
			<% } %>
				<button type="button" class="btn btn-primary btn-table" onClick="goUbiViewer('WEB-INF/jsp/welfare/<%=box.get("dbo")%>/report/s003.jrf', 'P_PARAM01!SLP002,<%=box.get("apply_no")%>!P_PARAM02!ACCTB_SLIP01|<%=obox.get("slip_no")%>|<%=obox.get("apply_no")%>|<%=obox.get("bugt_cd")%>,<%=obox.get("bugt_cd")%>!');"><img src="/com/img/ico_btn_print.png">인쇄</button>
			</td>
  		</tr>
	</table>
	<div class="table-bottom-control">
		...
		<% } %>
			<button type="button" class="btn dark" onClick="goUbiViewer('WEB-INF/jsp/welfare/<%=box.get("dbo")%>/report/s003.jrf', 'P_PARAM01!SLP002,<%=box.get("apply_no")%>!P_PARAM02!ACCTB_SLIP01|<%=obox.get("slip_no")%>|<%=obox.get("apply_no")%>|<%=obox.get("bugt_cd")%>,<%=obox.get("bugt_cd")%>!');"><img src="/com/img/ico_btn_print.png">인쇄</button>	
		</span>
	</div>
</form>
