...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th width="15%">신청구분</th>
			<td width="35%">
				<select name="data12" class="bs-select form-control required" style="width:40%" onChange="changeData12(this.value)">
					<option value=''>:: 선택 ::</option>
					<option value='부양비' <%=box.get("data12").equals("부양비")?"selected":""%>>부양비</option>
					<option value='의료비' <%=box.get("data12").equals("의료비")?"selected":""%>>의료비</option>
				</select>
			</td>
			<th width="15%">부양부모명</th>
			<td width="35%">
				<input type="text" name="data13" value="<%=obox.get("data13")%>" size="10" maxlength="15" title="자녀명" class="form-control required" style="width:100px" >
			</td>
		</tr>
		<tr>
			<th>신청일자</th>
		...
		<tr>
			...
			<td colspan="3">
				...
				<input type="text" name="account_no" value="<%=obox.get("account_no")%>" size="18" maxlength="30" title="계좌번호" <%if(!uSesEnt.corp_cd.equals("SABOK27")){%>class="form-control required"<%}%> style="width:45px">&nbsp;예금주 :
				<input type="text" name="account_owner" value="<%=obox.get("account_owner")%>" size="10" maxlength="15" title="예금주" <%if(!uSesEnt.corp_cd.equals("SABOK27")){%>class="form-control required"<%}%> style="width:45px">
			</td>
		</tr>
		<tr>
			<th>비고</th>
