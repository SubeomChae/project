...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("입양지원 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>신청구분</th>
			<td width="35%"><%=obox.get("data12")%></td>
			<th>자녀명</th>
			<td width="35%"><%=obox.get("data13")%></td>
		</tr>
		<tr>
			<th>신청일자</th>
			<td><%=obox.get("apply_ymd",Box.DEF_DATE_FMT)%></td>
			<th>신청금액</th>
			<td><%=obox.get("apply_amt",Box.THOUSAND_COMMA)%> 원 
		</tr>
		<tr>
			<th>계좌번호</th>
...
<input type="hidden" name="subject" 		value="입양지원 신청(<%=obox.get("empl_nm")%>)">
