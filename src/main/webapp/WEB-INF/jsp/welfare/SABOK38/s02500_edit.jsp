...
<script language="javascript">
	...
	function change_data12(data12) { 
		if(data12 == '') {
			document.main.apply_amt.value = '';
		}else if(data12 == '입양장려금') {
			document.main.apply_amt.value = '5000000';
		}else if(data12 == '양육비') {
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>신청구분</th>
			<td>
				<select name="data12" style="width:40%" class="bs-select form-control required" onChange="change_data12(this.value)">
					<option value=''>:: 선택 ::</option>
					<option value='입양장려금' <%=box.get("data12").equals("입양장려금")?"selected":""%>>입양장려금</option>
					<option value='양육비' <%=box.get("data12").equals("양육비")?"selected":""%>>양육비</option>
				</select>
			</td>
			<th>자녀명</th>
			<td>
