...
<script language="javascript">
	...
	function signRequestPost() {
		document.viewForm.action = 'wel01002_select.do';
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("양육비 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>자녀구분</th>
			<td><%=obox.get("data12")%></td>
			<th>자녀나이</th>
			<td><%=obox.get("data13").equals("")?"":obox.get("data13")+" 세"%></td>
		</tr>
		<tr>
			<th width="15%">신청일자</th>
	...
	<div class="table-bottom-control">
		...
		<% } %>
<%-- 			<button type="button" class="btn dark" onClick="goUbiViewer('WEB-INF/jsp/welfare/<%=box.get("dbo")%>/report/s005.jrf', 'P_PARAM01!SLP002,<%=box.get("apply_no")%>!P_PARAM02!ACCTB_SLIP01|<%=obox.get("slip_no")%>|<%=obox.get("apply_no")%>|<%=obox.get("bugt_cd")%>,<%=obox.get("bugt_cd")%>!');"><img src="/com/img/ico_btn_print.png">인쇄</button> --%>
			<button type="button" class="btn dark" onClick="goRptViewer('WEB-INF/jsp/welfare/<%=uSesEnt.dbo%>/report/s002PDF', '<%=box.get("apply_no")%>');"><img src="/com/img/ico_btn_print.png">인쇄</button>
		</span>
	</div>
</form>
</form>
...
<input type="hidden" name="subject" 		value="양육비 신청(<%=obox.get("empl_nm")%>)">
