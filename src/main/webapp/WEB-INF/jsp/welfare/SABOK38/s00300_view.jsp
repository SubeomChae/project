...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>학교명</th>
			<td colspan="3">
				<%=obox.get("school_nm")%><%=obox.get("school_flag_nm")%> <%=obox.get("school_year")%>학년 <%=obox.get("school_term")%><%if(!obox.get("school_term").equals("")){%>학기<%}%>
			</td>
		</tr>
		<tr>
			<th width="15%">학과</th>
	...
	<div class="table-bottom-control">
		<span class="f-right">
			...
			<% } %>
				<button type="button" class="btn dark" onClick="goRptViewer('WEB-INF/jsp/welfare/<%=uSesEnt.dbo%>/report/s003PDF', '<%=box.get("apply_no")%>');"><img src="/com/img/ico_btn_print.png">인쇄</button>
		</span>
</form>
<form name="signForm" method="post" action="" style="display:none">
<input type="hidden" name="send_empl_no" 	value="<%=uSesEnt.empl_no%>">
<input type="hidden" name="subject" 		value="장학금 신청(<%=obox.get("empl_nm")%>)">
