...
<% } %>
	<div class="table-top-guide">
		<span style="color:#01a1db;">대상자를 클릭</span>하면 해당 양육비 상세내역을 조회할 수 있습니다.
	</div>
		...
		<div class="col-md-5 col-xs-5 col-sm-5 row-set-col padL0 padR0">
			<div id="divArea" class="table-scrollable scrollOptionY" style="height:calc(100vh - 160px)">
				<table id="MyTable" class="table table-striped table-bordered table-hover order-column">	
					...
					<thead>
						<tr>
							<th><a href="javascript:goSort('t1.apply_ymd')" title="신청일순 정렬">신청일자</a></th>
							<th><a href="javascript:goSort('t1.data12')" title="신청구분 정렬">신청구분</a></th>
							<th><a href="javascript:goSort('t3.dept_nm')" title="부서명순 정렬">부서명</a></th>
							<th><a href="javascript:goSort('t2.empl_nm')" title="성명순 정렬">성명</a></th>
							<th><a href="javascript:goSort('t4.sign_status')">상태</a></th>
		          		</tr>
		          	</thead>
		          	<tbody>
					<% if(rsWp.getLength() == 0) { %>
							<tr height="25"><td colspan="5" class="t-center">등록된 자료가 없습니다.</td></tr>
					<%}else { 
						  for(int i=0; i < rsWp.getLength(); i++) { %>
							<tr id="tr_<%=i%>" apply_no="<%=rsWp.get("apply_no",i)%>" style="cursor:pointer" onClick="goDetail('<%=rsWp.get("apply_no",i)%>')"> 
								...
								<td class="t-center"><%=rsWp.get("data12",i)%>자녀(<%=rsWp.get("data13",i)%>세)</td>
