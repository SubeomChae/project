...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("노부모부양 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			...
			<td width="35%"><%=obox.get("data02")%></td>
			<th width="15%">요양등급</th>
		...
		<tr>
			...
			<th>요양기간</th>
			<td><%=obox.get("data14",Box.DEF_DATE_FMT)%> ~ <%=obox.get("data15",Box.DEF_DATE_FMT)%></td>
		</tr>
		<tr>
			<th>신청일자</th>
...
<input type="hidden" name="subject" 		value="노부모부양 신청(<%=obox.get("empl_nm")%>)">
