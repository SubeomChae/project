...
<script language="javascript">
	function goSave() {
		if (formValidate("#main")) {
			...
			var data12 = document.main.data12.value;
			var rtnMsg = '';
			if(data12 == '1등급' && document.main.apply_amt.value.replaceAll(',','') > 2133540) {
				rtnMsg = '1등급 지급한도(2,133,540원)를 초과하였습니다. ';
			}else if(data12 == '2등급' && document.main.apply_amt.value.replaceAll(',','') > 1879740) {
				rtnMsg = '2등급 지급한도(1,879,740원)를 초과하였습니다. ';
			}else if(data12 == '3등급' && document.main.apply_amt.value.replaceAll(',','') > 1736640) {
				rtnMsg = '3등급 지급한도(1,736,640원)를 초과하였습니다. ';
			}else if(data12 == '4등급' && document.main.apply_amt.value.replaceAll(',','') > 1626840) {
				rtnMsg = '4등급 지급한도(1,626,840원)를 초과하였습니다. ';
			}else if(data12 == '5등급' && document.main.apply_amt.value.replaceAll(',','') > 1379880) {
				rtnMsg = '5등급 지급한도(1,379,880원)를 초과하였습니다. ';
			}

			if(rtnMsg != '') {
				alert(rtnMsg);
				document.main.apply_amt.focus(); 
				return;
			}
	...
	function find_employee_no_post(empl_no, empl_nm) {
		...
		document.main.action = "<%=CD.CTPH%>wel01003_edit.do";
		document.main.target = "_self";
		document.main.submit();
	}		  
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th width="15%">요양대상자</th>
			<td width="35%">
				<input type="text" name="data02" value="<%=obox.get("data02")%>" size="10" maxlength="50" title="요양대상자" class="form-control required" style="width:100px">
			</td>
			<th width="15%">요양등급</th>
			<td width="35%">
				<select name="data12" class="bs-select form-control required" style="width:40%">
					<option value=''>:: 선택 ::</option>
					<option value='1등급' <%=obox.get("data12").equals("1등급")?"selected":""%>>1등급</option>
					<option value='2등급' <%=obox.get("data12").equals("2등급")?"selected":""%>>2등급</option>
					<option value='3등급' <%=obox.get("data12").equals("3등급")?"selected":""%>>3등급</option>
					<option value='4등급' <%=obox.get("data12").equals("4등급")?"selected":""%>>4등급</option>
					<option value='5등급' <%=obox.get("data12").equals("5등급")?"selected":""%>>5등급</option>
				</select>
			</td>
		</tr>
		<tr>
			<th>요양시설명</th>
			<td>
				<input type="text" name="data13" value="<%=obox.get("data13")%>" size="30" maxlength="50" title="요양시설명" class="form-control required" style="width:70%">
			</td>
			<th>요양기간</th>
			<td>
				<div class="form-inline">
					<div class="input-group input-medium date date-picker" data-date="<%=obox.get("data14",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" id="data14" name="data14" value="<%=obox.get("data14")%>" maxlength="8" title="요양기간(부터)" class="form-control required tYMD">
						<span class="input-group-btn"><button class="btn dark" type="button"><i class="fa fa-calendar"></i></button></span>
					</div>
					<span> ~ </span>
					<div class="input-group input-medium date date-picker" data-date="<%=obox.get("data15",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" id="data15" name="data15" value="<%=obox.get("data15")%>" maxlength="8" title="요양기간(까지)" class="form-control required tYMD">
						<span class="input-group-btn"><button class="btn dark" type="button"><i class="fa fa-calendar"></i></button></span>
					</div>&nbsp;까지
				</div>
			</td>
		...
		<tr>
			...
			<td colspan="3">
				...
				<input type="text" name="account_no" value="<%=obox.get("account_no")%>" size="18" maxlength="30" title="계좌번호" <%if(!uSesEnt.corp_cd.equals("SABOK27")){%>class="form-control required"<%}%> style="width:100px">&nbsp;예금주 :
