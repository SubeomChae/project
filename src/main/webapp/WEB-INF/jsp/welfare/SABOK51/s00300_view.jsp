...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("학자금 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="msg_popup" 	value="<%=box.get("msg_popup")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		...
		<tr>
			<th width="15%">학교소재지</th>
			<td width="35%"><%=obox.get("school_addr")%></td>
			<th width="15%">대상자</th>
			<td width="35%"><%=obox.get("family_rel_nm").equals("")?"":"["+obox.get("family_rel_nm")+"]"%> <%=obox.get("family_nm")%></td>					
		</tr>
		<tr>
			<th>생년월일</th>
			<td><%=obox.get("family_birth_ymd",Box.DEF_DATE_FMT)%></td>
			<th>학생과의관계</th>
			<td><%=obox.get("family_rel")%></td>
		</tr>
		<tr>
			<th>신청일자</th>
