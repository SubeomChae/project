	...
	function changeBugtCd(bugt_cd) {
		...
		if(bugt_cd == 'S001') {
			...
			cHtml = "<b>학교구분</b> : "
				  ...
				  + "&nbsp;&nbsp;&nbsp;&nbsp;<select name='detail_key' class='form-control' style='width:90px'>"
			...
			cHtml = "<select name='detail_key' class='bs-select form-control' style='width:90px'>"
				  ...
				  + "<input type='text' name='detail_val' class='form-control' value='' style='width:150px;ime-mode:active;' onkeydown=\"if(event.keyCode == 13) goSearch('N');\">";
	...
	<table class="search-panel table table-bordered">
		...
		<tbody>
			<tr>
				...
				<td>
					<select name="bugt_cd" title="목적사업" class="bs-select form-control" style="width:130px" onChange="changeBugtCd(this.value.substring(0,4));">
				...
				<td>
					<select name="sign_status" title="결재상태" class="bs-select form-control" style="width:90px">
						<option value=''>전체</option>
					...
					<%if(!box.get("excel_yn").equals("Y")){%>
						<%if(!bugt_cd.equals("S700") && !bugt_cd.equals("S008")){%>
							<%if(rsWp.get("biz_type",i).equals("A")){%>
								<a href="javascript:goSlipViewer('&slip_no=<%=rsWp.get("slip_no",i)%>');"'<%=rsWp.get("slip_no",i)%>');">
							<%}else{ %>
