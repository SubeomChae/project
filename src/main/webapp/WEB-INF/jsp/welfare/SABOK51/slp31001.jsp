	...
	<tr> 
		<td valign="top" style="padding:10px 10px 0 10px;">
	      <table width="100%" height="" cellpadding="0" cellspacing="0" border="0">
	        <tr>
	          <h1 class="page-title">회계전표 승인/취소</td>
	        </tr>
		  </table>
		</td>
		...
		<td align="center">
			<table class="table table-view table-bordered table-cover marB5">
		  		<tr>
					...
					<td width="10%" align="center"><%=box.get("sign_yn").equals("Y")?"회계일자":"접수일자"%></td>
					<td width="24%" nowrap>
					...
					<td width="10%" align="center">업무유형</td>
					<td>
						<select name="biz_type" style="width:160px">
							<option value=''>전체</option>
							<%=CodeUtil.optList("PR_COMTB_CODE02_LIST","LISTCODE",box.get("biz_type"),"biz_type","","","","", request)%>
						</select>
					</td>
					...
					<td width="*"><input type="text" name="search_val" value="<%=box.get("search_val")%>" style="width:120px; border:1 solid gray;ime-mode:active;" onkeydown="if(event.keyCode == 13) goSearch('N')"></td>
		  		</tr>
		  		<tr>
					<td align="center">전표번호</td>
					<td nowrap>
						...
						<% if(!box.get("slip_ymd").equals("")) { %>
							<%=CodeUtil.optList("PR_ACCTB_SLIP01_LIST","LISTCODE",box.get("slip_no"),box.get("slip_ymd"),"","","","3", request)%>
						<% } %>
						</select>
					</td>
					<td align="center">계정과목</td>
					<td nowrap>
						<input type="text" name="acct_nm" value="<%=box.get("acct_nm")%>" title="계정과목" style="width:132px; border:1 solid gray;ime-mode:active;" onKeyUp="document.main.acct_cd.value='';" onkeydown="if(event.keyCode == 13) goSearch('N');">
