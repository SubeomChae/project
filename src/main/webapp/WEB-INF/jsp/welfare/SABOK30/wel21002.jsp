	...
	<tr>
		...
		<% }else {
				...
				<% } %>
					...
					<td width="7%" class="t-center"><%if(!box.get("excel_yn").equals("Y")){%><%if(!bugt_cd.equals("S700") && !bugt_cd.equals("S008")){%><a href="javascript:go_welfare_view('<%=rsWp.get("apply_no",i)%>','<%=rsWp.get("bugt_cd",i)%>');"><%}else{%><a href="javascript:go_employee_info('<%=rsWp.get("empl_no",i)%>')"><%}%><%}%><%=rsWp.get("empl_nm",i)%></a></td>
