...
<table width="100%" border="0" cellspacing="0" bgcolor="#ffffff">
		...
		<td valign="top">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table01">
				<tr height="28">
					...
					<td colspan="3" style="padding-left:5px"><%=obox.get("school_nm")%><%=obox.get("school_flag_nm")%> <%=obox.get("school_year")%>학년 <%=obox.get("school_term")%><%if(!obox.get("school_term").equals("")){%>학기<%}%></td>
				</tr>
				<tr height="28">
					<td width="15%" align="center" class="table01_title">학과</td>
					<td width="35%" style="padding-left:5px"><%=obox.get("major_nm")%></td>
					<td width="15%" align="center" class="table01_title">대상자</td>
					<td width="35%" style="padding-left:5px">[<%=obox.get("family_rel_nm")%>] <%=obox.get("family_nm")%></td>					
				</tr>
			<% if(uSesEnt.dbo.equals("SABOK26")) { %>
				<tr height="28">
					<td align="center" class="table01_title">학점</td>
					<td style="padding-left:5px"><%=obox.get("grade_score")%></td>
					<td align="center" class="table01_title">납부금액</td>
					<td style="padding-left:5px"><%=obox.get("pay_amt",Box.THOUSAND_COMMA)%> 원</td>
				</tr>
			<% } %>
				<tr height="28">
					...
					<td style="padding-left:5px"><%=obox.get("apply_amt",Box.THOUSAND_COMMA)%> 원</td>
				</tr>
				<tr height="28">
					<td align="center" class="table01_title">계좌번호</td>
					<td colspan="3" style="padding-left:5px"><%=obox.get("bank_nm")%> <%=obox.get("account_no")%> (예금주 : <%=obox.get("account_owner")%>)</td>
				</tr>
				<tr height="28">
	...
	<tr>
		...
		<% } %>
			<a href="javascript:go_ubi_viewer('welfare/<%=box.get("dbo")%>/report/s003.jrf', 'P_PARAM01!SLP002,<%=box.get("apply_no")%>!P_PARAM02!ACCTB_SLIP01|<%=obox.get("slip_no")%>|<%=obox.get("apply_no")%>|<%=obox.get("bugt_cd")%>,<%=obox.get("bugt_cd")%>!');"><img src="/com/img/bn_print.gif" border="0" align="absmiddle" alt="인쇄"></a>
		</td>
	</tr>
</table>
</form>
