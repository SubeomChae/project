...
<script language="javascript">
	function goSearch(apply_no) {
		...
		ajaxReplaceForm("wel02001_list.do", "#main", "#divListData", function() {
			if(apply_no == '') {
	...
	function goWrite(){
		...
		ajaxReplaceForm("wel02003_edit.do", "#main", "#divFrame");
	}
	
	function goDetail(apply_no) {
		$("tr[id^='tr_']").removeClass("bg-yellow");
		if(apply_no != '') { $("tr[apply_no='"+apply_no+"']").addClass("bg-yellow"); }
		var pageUrl = 'wel02002_select.do?apply_no='+apply_no;
...
<input type="hidden" name="page_count"	value="">
	...
	<h1 class="page-title">직원 복지연금 신청</h1>
...
<% } %>
	<div class="table-top-guide">
		<span style="color:#01a1db;">신청일자를 클릭</span>하면 해당 복지연금 신청 세부내역을 조회할 수 있습니다.
	</div>
				...
				<table id="MyTable" class="table table-striped table-bordered table-hover order-column">	
					...
					<thead>
						<tr>
							...
							<th><a href="javascript:goSort('t1.support_item')">지원항목</a></th>
							<th><a href="javascript:goSort('t1.sign_status')">상태</a></th>
		          		</tr>
					</thead>
					<tbody>
					<% if(rsWp.getLength() == 0) { %>
						<t><td colspan="5" class="t-center">등록된 자료가 없습니다.</td></tr>
					<%}else { 
						for(int i=0; i < rsWp.getLength(); i++) { %>
						<tr id="tr_<%=i%>" apply_no="<%=rsWp.get("apply_no",i)%>" style="cursor:pointer" onClick="goDetail('<%=rsWp.get("apply_no",i)%>')"> 
							...
							<td class="t-center"><%=rsWp.get("support_item_nm",i)%></td>
