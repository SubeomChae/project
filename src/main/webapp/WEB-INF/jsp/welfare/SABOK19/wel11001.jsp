		...
		<div class="table-top-control">
			...
			<div class="form-inline f-right">
				<div class="form-group">
			  	...
			  	<% }else if(isEditable) { %>
			  		<b>총 지급액</b> : 
					<input type="text" name="member_cnt" value="<%=obox.getInt("member_cnt")%>" size="4" title="인원수" class="form-control acc-border tMONEY" style="width:40px" readOnly> 명 x 
					<input type="text" name="pay_amt" value="<%=pbox.get("unit_amt")%>" size="10" title="지원금액" class="form-control acc-border tMONEY" style="width:80px" readOnly> 원   
					- (할인액 : <input type="text" name="discount_amt" value="<%=pbox.getInt("discount_amt")%>" size="10" title="할인액" class="form-control acc-border tMONEY" style="width:80px" onChange="document.form1.pay_amt_tot.value = parseInt2(document.form1.pay_amt_tot) - parseInt(this.value); formatMaskElm(document.form1.pay_amt_tot);"> 원) 
					= <input type="text" name="pay_amt_tot" value="<%=obox.getInt("member_cnt")*pbox.getLong("unit_amt")%>" size="13" title="합계" class="form-control acc-border tMONEY" style="width:80px" readOnly> 원
			  	<% } %>
			 	</div>
			 </div>
		</div>
						...
						<%if(isEditable && !box.get("excel_yn").equals("Y")){%>
							<th>
								<label class="mt-checkbox mt-checkbox-outline">
									<input type="checkbox" name="checkbox" value="Y" style="border:none" onClick="clickCheckAll(this.checked)"><span/>
								</label>
							</th>
						<% } %>
	          		</tr> 
				...
				<% }else if(rsWp.getLength() > 0) {
					   String old_empl_no = rsWp.get("sort_key",0) + "@" + rsWp.get("sort_key",rsWp.getLength()-1);
						...
						<tr> 
							...
							<%if(isEditable && !box.get("excel_yn").equals("Y")){%>
								<td class="t-center">
									<input type="checkbox" name="new_empl_no<%=i%>" value="<%=rsWp.get("empl_no",i)%>," style="border:none" <%=rsWp.get("exist_yn",i).equals("Y")?"checked":""%> onClick="autoCalc('<%=i%>')">
								</td>
							<%}%>
						</tr>	
					<% } %>
