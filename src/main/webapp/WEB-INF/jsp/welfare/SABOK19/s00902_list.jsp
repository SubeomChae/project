...
<!-- <%if(uSesEnt.dbo.equals("SABOK19")){%>자녀양육비<%}else{%>유아자녀교육비<%}%> -->
...
<input type="hidden" name="selectedRow" value="">
	...
	<h1 class="page-title"><%if(uSesEnt.dbo.equals("SABOK19")){%>자녀양육비<%}else{%>유아자녀교육비<%}%> 신청</h1>
...
<% } %>
	<div class="table-top-guide">
 		<span style="color:#01a1db;">신청일자를 클릭</span>하면 해당 <%if(uSesEnt.dbo.equals("SABOK19")){%>자녀양육비<%}else{%>유아자녀교육비<%}%> 신청 세부내역을 조회할 수 있습니다.
 	</div>
		...
		<div class="col-md-5 col-xs-5 col-sm-5 row-set-col padL0 padR0">
			<div id="divArea" class="table-scrollable scrollOptionY" style="height:calc(100vh - 160px)">
				<table id="MyTable" class="table table-striped table-bordered table-hover order-column">	
					...
					<%}else { 
						  for(int i=0; i < rsWp.getLength(); i++) { %>
							<tr id="tr_<%=i%>" apply_no="<%=rsWp.get("apply_no",i)%>" style="cursor:hand" onClick="goDetail('<%=rsWp.get("apply_no",i)%>')"> 
