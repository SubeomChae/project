...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("장학금 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="msg_popup" 	value="<%=box.get("msg_popup")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>학교명</th>
			<td width="35%"><%=obox.get("school_nm")%><%=obox.get("school_year")%>학년 <%=obox.get("school_term")%><%if(!obox.get("school_term").equals("")){%>학기<%}%></td>
			<th>대상자</th>
		...
		<tr>
			...
			<td>
				...
				<%if(obox.get("sign_status").equals("2") && box.get("msg_popup").equals("Y") && uSesEnt.use_auth.startsWith("S")){%>
					(지원금액 : <input type="text" name="apply_amt" value="<%=obox.get("apply_amt")%>" size="13" maxlength="10" title="지원금액" class="required tMONEY" style="width:70px;border:1 solid gray" > 원)
				...
				<% }else if(!obox.get("apply_amt").equals(obox.get("support_amt"))){%>
					(실수령금액 : <%=obox.get("apply_amt",Box.THOUSAND_COMMA)%> 원)
				<%}%>
			</td>
		</tr>
		<tr>
