...
<%	String apply_no = box.get("params");
	Box obox = CodeHelper.getRsBox("PR_REPORT_SQL", "WEL01001", apply_no, "S022","","","",request);
	...
	Paragraph title = new Paragraph("모성보호신청서", font_title);
	...
	table.addCell(PdfUtil.cellFormat("출산예정일", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("due_date",Box.DEF_DATE_FMT), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
	...
	if(obox.get("sign_status").equals("3")) {
		...
		table2.addCell(PdfUtil.cellFormat(obox.get("apply_amt",Box.THOUSAND_COMMA)+" 원", font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
		table2.addCell(PdfUtil.cellFormat("이전지원횟수", font_head, "", "", "LIGHT_GRAY", "m25", ""));
		table2.addCell(PdfUtil.cellFormat(StringUtil.arrayCol(obox.get("etc_remark"),1).equals("")?"":StringUtil.arrayCol(obox.get("etc_remark"),1)+" 회", font_data, "LEFT", "", "", "m25", "0,5,0,5"));
		table2.addCell(PdfUtil.cellFormat("증빙확인", font_head, "", "", "LIGHT_GRAY", "m25", ""));
		table2.addCell(PdfUtil.cellFormat(StringUtil.arrayCol(obox.get("etc_remark"),2), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
		table2.addCell(PdfUtil.cellFormat("비고", font_head, "", "", "LIGHT_GRAY", "m25", ""));
		table2.addCell(PdfUtil.cellFormat(StringUtil.arrayCol(obox.get("etc_remark"),3), font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
