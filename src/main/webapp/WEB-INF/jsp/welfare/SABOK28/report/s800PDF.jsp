...
<title>직원 복지연금 신청서</title>
</head>
<body>
<%	String apply_no = box.get("params");
	Box obox = CodeHelper.getRsBox("PR_REPORT_SQL", "S80000", apply_no, "","","","",request);
	...
	Paragraph title = new Paragraph("직원 복지연금 신청서", font_title);
	...
	if(obox.get("sign_status").compareTo("2") >= 0) {
		// 결재선
		String search_key = obox.get("apply_no")+"|"+obox.get("bugt_cd")+"|"+obox.get("support_item")+"|"+obox.get("empl_no");
		Box msgBox = CodeHelper.getRsBox("PR_REPORT_SQL", "SIGNLINE", "BOK001", search_key, "","","",request);
	...
	table.addCell(PdfUtil.cellFormat("지원항목", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("support_item_nm"), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
	table.addCell(PdfUtil.cellFormat("본인부담금", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("self_need_amt",Box.THOUSAND_COMMA)+"원", font_data, "LEFT", "", "", "m25", "0,5,0,5"));
	...
	if(obox.get("sign_status").equals("3")) {
		table2.addCell(PdfUtil.cellFormat("담당자기재란", font_head, "", "", "LIGHT_GRAY", "m25", "0,5,0,5", "col4"));
		table2.addCell(PdfUtil.cellFormat("지원금액", font_head, "", "", "LIGHT_GRAY", "m25", ""));
		table2.addCell(PdfUtil.cellFormat(obox.get("support_amt",Box.THOUSAND_COMMA)+"원", font_data, "LEFT", "", "", "m25", "0,5,0,5"));
		table2.addCell(PdfUtil.cellFormat("증빙확인", font_head, "", "", "LIGHT_GRAY", "m25", ""));
		table2.addCell(PdfUtil.cellFormat(StringUtil.arrayCol(obox.get("etc_remark"),1), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
