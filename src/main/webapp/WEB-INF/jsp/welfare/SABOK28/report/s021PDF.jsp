...
<title>선진산업시찰신청서</title>
</head>
<body>
<%	String apply_no = box.get("params");
	Box obox = CodeHelper.getRsBox("PR_REPORT_SQL", "WEL01001", apply_no, "S021","","","",request);
	...
	Paragraph title = new Paragraph("선진산업시찰신청서", font_title);
	...
	table.addCell(PdfUtil.cellFormat("예정지역", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("area_nm"), font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
	table.addCell(PdfUtil.cellFormat("일정", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(!obox.get("end_ymd").equals(obox.get("start_ymd"))?obox.get("start_ymd",Box.DEF_DATE_FMT)+" ~ "+obox.get("end_ymd",Box.DEF_DATE_FMT)+"  (이용일수 : "+obox.get("use_ilsu")+" 일)":obox.get("start_ymd",Box.DEF_DATE_FMT)+"  (이용일수 : "+obox.get("use_ilsu")+" 일)", font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
	table.addCell(PdfUtil.cellFormat("주관사", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("agent_nm"), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
	...
	if(obox.get("sign_status").equals("3")) {
		...
		table2.addCell(PdfUtil.cellFormat("지원대상\n해당여부", font_head, "", "", "LIGHT_GRAY", "m25", ""));
		table2.addCell(PdfUtil.cellFormat(StringUtil.arrayCol(obox.get("etc_remark"),1).equals("")?"":StringUtil.arrayCol(obox.get("etc_remark"),1).equals("1")?"5년 근속/정년(임기만료) 1년 이내":"10년 근속/정년(명예퇴직) 신청", font_data, "LEFT", "", "", "m25", "0,5,0,5"));
