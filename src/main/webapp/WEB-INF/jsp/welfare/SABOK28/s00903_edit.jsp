...
<script language="javascript">
	function goSave() {
		if (formValidate("#saveForm")) {
			if(confirm('저장하시겠습니까?')) {
				removeMask("#saveForm");
				document.saveForm.data09.value = document.saveForm.apply_people.value;
	...
	function changeSupportAmt(apply_people) {		
		var amt = '';
		if(apply_people == 2) {
			amt = 500000;
		}else if(apply_people == 3) {
			amt = 700000;
		}else if(apply_people > 3){
			amt = 700000+(300000*(apply_people-3));
		}
		
		document.saveForm.support_amt.value = amt;
...
<%for(int i=1;i<=15;i++){%><input type="hidden" name="data<%=StringUtil.lpad(String.valueOf(i),"0",2)%>" value=""><%}%>
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>신청인원</th>
			<td colspan="3">
				<select name="apply_people" title="신청인원" class="bs-select form-control required" style="width:100px" onChange="changeSupportAmt(this.value)">
					<option value=''>:: 선택 ::</option>
				<% for(int i=2; i < 12; i++) { %>
					<option value='<%=i%>' <%=obox.get("apply_people").equals(i)?"selected":""%>><%=i%>명</option>
				<%	} %>
				</select>
			</td>
		</tr>
		<tr>
			...
			<th width="15%">신청금액</th>
			<td width="35%">
				<input type="text" name="support_amt" value="<%=obox.get("support_amt")%>" size="13" maxlength="10" title="신청금액" class="form-control tMONEY" style="width:90px" readOnly> 원
			</td>
		</tr>
		<tr>
			<th>계좌번호</th>
