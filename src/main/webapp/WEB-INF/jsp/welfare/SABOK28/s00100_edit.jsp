...
<%for(int i=1;i<=15;i++){%><input type="hidden" name="data<%=StringUtil.lpad(String.valueOf(i),"0",2)%>" value=""><%}%>
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th width="15%">경조구분</th>
			<td width="35%">
				<select name="cong_class" class="bs-seelct form-control required" title="경조구분" style="width:90px" onChange="autoCongAmt()">
			...
			<th width="15%">대상자</th>
			<td width="35%">
				<select name="family_id" class="bs-select form-control required" title="대상자 선택" style="width:125px" onChange="changeFamilyId(this.value)">
					<option value=''>:: 선택 ::</option>
				...
				<input type="text" id="family_nm" name="family_nm" value="<%=obox.get("family_nm")%>" size="10" maxlength="8" class="form-control required" style="display:none;">
	...
	<% if(!box.get("apply_no").equals("")) { %>
  		<tr>
			<th>전표번호</th>
			<td><%=obox.get("slip_no",Box.SLIP_NO)%>
				<%if(!obox.get("slip_no").equals("")) { %>
					<a href="javascript:goSlipViewer('<%=obox.get("slip_no")%>');"><img src="/com/img/icon_zoom.gif" align="absmiddle" border="0"></a>
				<% } %>
				<input type="hidden" name="slip_no" 	value="<%=obox.get("slip_no")%>">
				<input type="hidden" name="sign_status" value="<%=obox.get("sign_status")%>">
			</td>
			<th class="t-center">결재상태</td>
