...
<script language='javascript'>
	...
	function changeBugtCd(bugt_cd) {
		...
		}else if(bugt_cd == 'S009') {
			cHtml = "";
			document.main.excel_file_nm.value = '다자녀가정 생활 지원현황';
		}else if(bugt_cd == 'S021') {
			cHtml = "";
			document.main.excel_file_nm.value = '선진산업시찰 지원현황';
		}else if(bugt_cd == 'S022') {
			cHtml = "";
			document.main.excel_file_nm.value = '모성보호 지원현황';
		}else if(bugt_cd == 'S024') {
			cHtml = "";
			document.main.excel_file_nm.value = '주택임차 및  구입비용 지원현황';
		...
		}else if(bugt_cd == 'S800') {
			cHtml = "";
			document.main.excel_file_nm.value = '직원 복지연금 지급현황';
		}else if(bugt_cd == '') {
			cHtml = "";
	...
	<table class="search-panel table table-bordered">
		...
		<tbody>
	  		<tr>
				...
				<td>
					<select name="bugt_cd" title="목적사업" class="bs-select form-control" style="width:130px" onChange="changeBugtCd(this.value);">
				...
				<td>
					<select name="sign_status" title="결재상태" class="bs-select form-control" style="width:100px">
						<option value=''>전체</option>
		...
		<table id="MyTable" class="table table-bordered table-hover order-column">
			...
			<% int colspan = 5; String bugt_cd = StringUtil.substr(box.get("bugt_cd"),0,4);%>
				...
				<% } %>
					...
					<th>부서명</th>
					<th>직위</th>
				<% if(!bugt_cd.equals("S700") && !bugt_cd.equals("S800")) { colspan += 2; %>
					<th>신청일자</th>
					<th>신청금액</th>
				<% } %>
				...
				<% }else if(bugt_cd.equals("S002")) { colspan += 5; %>	
					...
					<th>의료기관명</th>
					<th>입원기간</th>
				<% }else if(bugt_cd.equals("S003")) { colspan += 4; %>	
					<th>가족관계/성명</th>
					<th>주민등록번호</th>
					<th>학교명</th>
					<th>학년/학기</th>
				<% }else if(bugt_cd.equals("S009")) { colspan += 3; %>
					<th>가족관계/성명</th>
					<th>주민등록번호</th>
					<th>신청인원</th>
				<% }else if(bugt_cd.equals("S021")) { colspan += 3; %>
					<th>예정지역</th>
					<th>기간</th>
					<th>비고</th>
				<% }else if(bugt_cd.equals("S022")) { colspan += 3; %>
					<th>출산예정일</th>
					<th>임산부 건강상태</th>
					<th>태아 건강상태</th>				
				<% }else if(bugt_cd.equals("S024")) { colspan += 3; %>
					<th>지원항목</th>
					<th>지원금액</th>
					<th>비고</th>
				<% }else if(bugt_cd.equals("S700") || bugt_cd.equals("S800")) { colspan += 6; %>
					<th>입사일자</th>
					<th>주민번호</th>
					<th>사원구분</th>
					<th>재직상태</th>
			...
			<% }else {
				...
				<% } %>
					...
					<td width="7%" class="t-center"><%if(!box.get("excel_yn").equals("Y")){%>
						<%if(!bugt_cd.equals("S007") && !bugt_cd.equals("S008")){%>
				...
				<% if(!bugt_cd.equals("S700") && !bugt_cd.equals("S800")) { %>	
				...
				<% }else if(bugt_cd.equals("S009")) { // 다자녀가정 생활 지원   %>
					<td class="t-center"><%=rsWp.get("family_rel_nm",i)%>/<%=rsWp.get("family_nm",i)%></td>
					<td class="t-center"><%=FormatUtil.juminNo(rsWp.get("family_reside_no",i),"x")%></td>
					<td class="t-center"><%=rsWp.get("apply_people",i)%> 명</td>
				<% }else if(bugt_cd.equals("S021")) { // 선진산업시찰 지원   %>
					<td width="10%" class="t-center"><%=rsWp.get("area_nm",i)%></td>
					<td width="15%" class="t-center"><%=rsWp.get("start_ymd",i,Box.DEF_DATE_FMT)%> ~ <%=rsWp.get("end_ymd",i,Box.DEF_DATE_FMT)%></td>
					<td width="*" class="t-center"><%=rsWp.get("etc_remark",i)%></td>
				<% }else if(bugt_cd.equals("S022")) { // 모성보호 지원   %>
					<td class="t-center"><%=rsWp.get("due_date",i,Box.DEF_DATE_FMT)%></td>
					<td class="t-center"><%=rsWp.get("mother_status",i)%></td>
					<td class="t-center"><%=rsWp.get("fetus_status",i)%></td>
				<% }else if(bugt_cd.equals("S024")) { // 주택임차 및 구입비용 지원   %>
					<td width="15%" class="t-center"><%=rsWp.get("support_item_nm",i)%></td>
					<td width="10%" class="t-center"><%=rsWp.get("support_amt",i,Box.THOUSAND_COMMA)%></td>
					<td width="*" class="t-center"><%=rsWp.get("etc_remark",i)%></td>
				<% }else if(bugt_cd.equals("S700") || bugt_cd.equals("S800")) { // 선물지급, 연금지급  %>
