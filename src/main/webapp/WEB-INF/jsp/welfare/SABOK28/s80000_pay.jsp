...
<script language='javascript'>
	function goSearch(gbn) {
		if(formValidate("#main")) {		
			var theURL = 'wel03001_list.do';
			if(gbn == 'E') { theURL = theURL + '&excel_yn=Y&excel_file_nm=직원 복지연금 일괄지급'; }
	...
	%>	
		if(inx == '') {
			document.main.member_cnt.value = document.form1.checkbox.checked ? <%=cnt+rsWp.getLength()%> : <%=cnt%>;
		}else {
...
<% if(!box.get("msg_popup").equals("Y")) { %>
	<h1 class="page-title">직원 복지연금 일괄지급</h1>
<% } %>
	<table class="search-panel table table-bordered">
		...
		<% if(!box.get("msg_popup").equals("Y")) { %>
	  		<tr>
				...
				<td>
					<select name="pay_yy" title="지급년도" class="bs-select form-control" style="width:70px" onChange="changePayYmd(this.value, document.main.pay_term.value)">
				...
				<td>
					<select name="support_item" class="bs-select form-control required" title="지원항목" style="width:170px" onChange="goSearch('N')">
						<%=CodeUtil.optList("PR_COMTB_CODE02_LIST","LISTCODE",box.get("support_item"),"support_item@",box.get("bugt_cd"),"","","")%>
					</select>
				</td>
				<td class="td-head">
				...
				<td>
					<input type="text" name="search_val" value="<%=box.get("search_val")%>" class="form-control" style="width:100px;ime-mode:active;" onkeydown="if(event.keyCode == 13) goSearch();">
				</td>
	  		</tr>
	  	<% } %>	
	  		<tr>
				...
				<% }else { %>
					...
					<%=obox.get("slip_no",Box.SLIP_NO)%><a href="javascript:goSlipViewer('&slip_no=<%=obox.get("slip_no")%>');"><b><%=obox.get("slip_no",Box.SLIP_NO)%></b></a>
					...
					<%if(isEditable){%>
						<th>
							<label class="mt-checkbox mt-checkbox-outline">
								<input type="checkbox" id="checkAll" name="checkAll" value="Y" onClick="goSelectAll();"><span/>
							</label>
						</th>
					<%}%>
	          		</tr>
				...
				<% }else if(rsWp.getLength() > 0) {
			   		String old_empl_no = rsWp.get("sort_key",0) + "@" + rsWp.get("sort_key",rsWp.getLength()-1);
						...
						<tr> 
							...
							<td class="t-center"><%if(!rsWp.get("msg_no",i).equals("")){%><a href="javascript:goDocuViewer('<%=rsWp.get("msg_no",i)%>');"><%=FormatUtil.msgNo(rsWp.get("msg_no",i))%></a><%}%></td>
						<%if(isEditable){%>
							<td class="t-center">
								<label class="mt-checkbox mt-checkbox-outline">
