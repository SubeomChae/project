	...
	<tr>
		...
		<% }else {
				...
				<% } %>
					...
					<td width="7%" class="t-center"><%if(!box.get("excel_yn").equals("Y")){%><%if(!bugt_cd.equals("S007") && !bugt_cd.equals("S008")){%><a href="javascript:go_welfare_view('<%=rsWp.get("apply_no",i)%>','<%=rsWp.get("bugt_cd",i)%>');"><%}else{%><a href="javascript:go_employee_info('<%=rsWp.get("empl_no",i)%>')"><%}%><%}%><%=rsWp.get("empl_nm",i)%></a></td>
				...
				<% }else if(bugt_cd.equals("S021")) { // 선진산업시찰 지원   %>
					<td width="10%" class="t-center" style="letter-spacing:-1px"><%=rsWp.get("area_nm",i)%></td>
