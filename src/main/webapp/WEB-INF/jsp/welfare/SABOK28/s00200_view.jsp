...
<input type="hidden" name="msg_popup" 	value="<%=box.get("msg_popup")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th width="15%">대상자</th>
			<td colspan="3"><%=obox.get("family_rel_nm").equals("")?"":"["+obox.get("family_rel_nm")+"]"%> <%=obox.get("family_nm")%></td>
		</tr>
		<tr>
			<th>진료기간</th>
		...
		<tr>
			...
			<td width="35%">
				...
				<% }else if(!obox.get("apply_amt").equals(obox.get("support_amt")) && !obox.get("sign_status").equals("1")){%>
