			...
			<% if(obox.getInt("member_cnt") > 0) { %>
				<tr height="28">
					...
					<% if(obox.get("slip_no").equals("")) { %>
						미작성&nbsp;<a href="javascript:goCreateSlip();"><img src="/com/img/bn_create_slip.gif" border="0" align="absmiddle" alt="전표분개"></a>
					<% }else { %>
						...
						<% if(obox.get("sign_status").equals("1")) { %>
							...
							<img src="/com/img/bn_delete_slip.gif" border="0" align="absmiddle" alt="전표삭제" style="cursor:hand" onClick="goDeleteSlip();">
					...
					<% }else if(rsWp.getLength() > 0) {
						   String old_empl_no = rsWp.get("sort_key",0) + "@" + rsWp.get("sort_key",rsWp.getLength()-1);
							...
							<tr height="25" onMouseOver="this.className='btnover'" onMouseOut="this.className='btnbase'" class="buttonbase"> 
								...
								<%if(isEditable){%><td align="center"><input type="checkbox" name="new_empl_no<%=i%>" value="<%=rsWp.get("empl_no",i)%>," style="border:none" <%=rsWp.get("exist_yn",i).equals("Y")?"checked":""%> onClick="autoCalc('<%=i%>')"></td><%}%>
							</tr>	
						<% } %>
