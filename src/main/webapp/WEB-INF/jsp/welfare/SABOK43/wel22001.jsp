	...
	<table class="search-panel table table-bordered">
		...
		<tbody>
	  		<tr>
				...
				<td>
					<select name="std_yy" title="기준년도" class="bs-select form-control" style="width:120px" onChange="goSearch('N')">
				...
				<td>
					<input type="text" name="search_val" value="<%=box.get("search_val")%>" class="form-control" style="width:150px;" onkeydown="if(event.keyCode == 13) goSearch('N');">
			...
			<% }else {
				...
				<tr> 
		   	    <% } %>
					...
					<td><%if(!rsWp.get("empl_nm",i).equals("합계")){%><a href="javascript:goEmployeePop('&empl_no=<%=rsWp.get("empl_no",i)%>')"><%}%><%=rsWp.get("empl_nm",i)%></a></td>
					<td><%=rsWp.get("dept_nm",i)%></td>
				<% for(int k=0; k < codeWp.getLength(); k++) { %>	
					<td><%=rsWp.get("amt_"+codeWp.get("bugt_cd",k).toLowerCase(),i,Box.THOUSAND_COMMA)%></td>
				<% } %>	
					<td><%=rsWp.get("amt_tot",i,Box.THOUSAND_COMMA)%></td>
				</tr>	
				<% } %>
			<% } %>
			</tbody>
