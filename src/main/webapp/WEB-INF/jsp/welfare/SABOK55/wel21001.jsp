...
<script language='javascript'>
	...
	function changeBugtCd(bugt_cd) {
		...
		if(bugt_cd == 'S001') {
			cHtml = "<b>경조구분</b> : "
				  + "<select name='detail_sel' class='bs-select form-control' style='width:130px'>"
			      + "<option value=''>전체</option><%=CodeUtil.optListJs(codeWp,"cong_class","")%></select>&nbsp;"
				  + "<select name='detail_key' class='bs-select form-control' style='width:130px'>"
			...
			cHtml = "<select name='detail_key' class='bs-select form-control' style='width:130px'>"
			...
			cHtml = "<b>동호인회</b> : "
				  + "<select name='detail_key' class='bs-select form-control' style='width:170px'>"
			...
			document.main.excel_file_nm.value = '장기근속 지원현황';
		}else if(bugt_cd == 'S021') {
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='bs-select form-control' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeUtil.optList("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S021","","","",uSesEnt.dbo)%></select> ";
			document.main.excel_file_nm.value = '워크샵';		
		}else if(bugt_cd == '') {
			cHtml = "";
...
<% } %>
	<div class="table-top-control form-inline">
		<div class="notice-view">
			<span style="font-weight:bold; color:#01a1db;">성명을 클릭</span>하면 해당 사원의 <span id="divText"></span>&nbsp;조회할 수 있습니다.
		</div>
		<span class="f-right">
		...
		<table id="MyTable" class="table table-bordered table-hover order-column">
			...
			<% int colspan = 5; String bugt_cd = StringUtil.substr(box.get("bugt_cd"),0,4);%>
				...
				<% } %>
					...
					<th>부서명</th>
					<th>직위</th>
				<% if(!bugt_cd.equals("S700") && !bugt_cd.equals("S008")) { colspan += 2; %>
					<th>신청일자</th>
					<th>신청금액</th>
				<% } %>
				<% if(bugt_cd.equals("S001")) { colspan += 5; %>	
					...
					<th>경조일자</th>
					<th>경조장소</th>
				<% }else if(bugt_cd.equals("S002")) { colspan += 5; %>	
					<th>가족관계/성명</th>
					<th>주민등록번호</th>
					<th>질병명</th>
					<th>의료기관명</th>
					<th>입원기간</th>
				<% }else if(bugt_cd.equals("S003") || bugt_cd.equals("S024") || bugt_cd.equals("S025")) { colspan += 5; %>	
					<th>가족관계/성명</th>
					<th>주민등록번호</th>
					<th>학교명</th>
					<th>학년/학기</th>
				...
				<% }else if(bugt_cd.equals("S004")) { colspan += 5; %>
					...
					<th>기관연락처</th>
					<th>기관주소</th>
				<% }else if(bugt_cd.equals("S005")) { colspan += 3; %>
					<th>휴양시설명</th>
					<th>기간</th>
				<% }else if(bugt_cd.equals("S006")) { colspan += 4; %>
					<th>동호인회</th>
					<th>행사일자</th>
					<th>장소</th>
					<th>행사내용</th>
				<% }else if(bugt_cd.equals("S700") || bugt_cd.equals("S008")) { colspan += 6; %>
					<th>입사일자</th>
					<th>주민번호</th>
					<th>사원구분</th>
					<th>재직상태</th>
					<th>지급일자</th>
					<th>지급액</th>
				<% }else if(bugt_cd.equals("S022") || bugt_cd.equals("S023")) { colspan += 2; %>
					<th>질병명</th>
					<th>입원기간</th>							
				<% } %>
				<% if(bugt_cd.equals("")) { %>
			...
			<% }else {
				...
				<tr>
				<% if(bugt_cd.equals("")) { %>
					<td width="8%" class="t-center"><%=rsWp.get("bugt_nm",i)%></td>
				<% } %>
					<td width="6%" class="t-center"><%=rsWp.get("empl_no",i)%></td>
					<td width="6%" class="t-center">
					<%if(!box.get("excel_yn").equals("Y")){%>
						<%if(!bugt_cd.equals("S700") && !bugt_cd.equals("S008")){%>
							<%if(rsWp.get("biz_type",i).equals("A")){%>
								<a href="javascript:goSlipViewer('&slip_no=<%=rsWp.get("slip_no",i)%>');">
							<%}else{ %>
					...
					<td width="15%" class="t-center"><%=rsWp.get("dept_nm",i)%></td>
					<td width="7%" class="t-center"><%=rsWp.get("position_nm",i)%></td>
				<% if(!bugt_cd.equals("S700") && !bugt_cd.equals("S008")) { %>	
					<td width="6%" class="t-center"><%=rsWp.get("apply_ymd",i,Box.DEF_DATE_FMT)%></td>
					<td width="8%" class="t-center"><%=rsWp.get("apply_amt",i,Box.THOUSAND_COMMA)%></td>
				<% } %>	
				<% if(bugt_cd.equals("S001")) { // 경조금  %>	
					<% if(rsWp.get("biz_type",i).equals("A")) {%>
					<td colspan="5" style="padding-left:5px"><%=rsWp.get("subject",i)%></td>
					<% }else {%>
					...
					<% } %>
					<% }else if(bugt_cd.equals("S002")) { // 의료비   %>
					<td class="t-center"><%=rsWp.get("family_rel_nm",i)%>/<%=rsWp.get("family_nm",i)%></td>
					<td class="t-center"><%=FormatUtil.juminNo(rsWp.get("family_reside_no",i),"x")%></td>
					...
					<% }else if(bugt_cd.equals("S003") || bugt_cd.equals("S024") || bugt_cd.equals("S025")) { // 학자금   %>
					<td class="t-center"><%=rsWp.get("family_rel_nm",i)%>/<%=rsWp.get("family_nm",i)%></td>
					<td class="t-center"><%=FormatUtil.juminNo(rsWp.get("family_reside_no",i),"x")%></td>
					...
					<% }else if(bugt_cd.equals("S004")) { // 자기개발비   %>
					...
					<% }else if(bugt_cd.equals("S005")) { // 휴양시설이용   %>
					<td class="t-center"><%=rsWp.get("condo_nm",i)%></td>
					<td class="t-center"><%=rsWp.get("start_ymd",i,Box.DEF_DATE_FMT)%> ~ <%=rsWp.get("end_ymd",i,Box.DEF_DATE_FMT)%></td>
					<% }else if(bugt_cd.equals("S006")) { // 사내동호인회 지원   %>
					...
					<% }else if(bugt_cd.equals("S700") || bugt_cd.equals("S008")) { // 선물지급, 연금지급  %>
					...
					<% }else if(bugt_cd.equals("S021") ) { // 워크샵  %>
					<td class="t-center"><%=rsWp.get("entry_ymd",i, Box.DEF_DATE_FMT)%></td>
					<td class="t-center"><%=FormatUtil.juminNo(rsWp.get("reside_no",i),"x")%></td>
					<td class="t-center"><%=rsWp.get("empl_class_nm",i)%></td>
					<td class="t-center"><%=rsWp.get("empl_status_nm",i)%></td>
					<td class="t-center"><%=rsWp.get("pay_ymd",i,Box.DEF_DATE_FMT)%></td>
					<td class="t-center"><%=rsWp.get("pay_amt",i,Box.THOUSAND_COMMA)%></td>
					<% }else if(bugt_cd.equals("S010")) { %>
					<td width="7%" class="t-center"><%=rsWp.get("start_ymd",i,Box.DEF_DATE_FMT)%></td>
					<td width="*" class="t-center"><%=rsWp.get("etc_remark",i)%></td>
					<% }else if(bugt_cd.equals("S022") || bugt_cd.equals("S023")) { %>
					<td width="7%" class="t-center"><%=rsWp.get("start_ymd",i,Box.DEF_DATE_FMT)%></td>
					<td width="*" class="t-center"><%=rsWp.get("etc_remark",i)%></td>
					<% } %>
					<% if(bugt_cd.equals("")) { %>
					<td width="10%" class="t-center"><%=rsWp.get("biz_type",i).equals("A")?rsWp.get("subject",i):rsWp.get("etc_remark",i)%></td>
					<td width="10%" class="t-center"><a href="javascript:goSlipViewer('&slip_no=<%=rsWp.get("slip_no",i)%>');"><%=rsWp.get("slip_no",i,Box.SLIP_NO)%></a></td>
					<% } %>
