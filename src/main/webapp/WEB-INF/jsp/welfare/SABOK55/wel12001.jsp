...
<script language='javascript'>
	...
	<% if(rsWp.getLength() > 0) { %> 
		document.main.unit_count.value = document.form1.unit_count.value;
	<% } %>
		ajaxReplaceForm("wel12001_list.do", "#main", "#divListData", function() {
			fixTableHeader("MyTable");
		});
		return false;
	}
	...
	function goCreateSlip() {
		if(formValidate("#main")) {
			if(confirm('전표를 생성하시겠습니까?')) {
				removeMask("#main");				
				document.main.action = 'wel12001_create_slip.do';
				document.main.target = '_self';
				document.main.submit();
			}
		}
	...
	function goDelete() {
		if(formValidate("#main")) {
			if(confirm('워크샾 대상자를 일괄 삭제하시겠습니까?')) {
				removeMask("#main");				
				document.main.action = 'wel12001_delete.do';
				document.main.target = '_self';
				document.main.submit();
			}
		}
	...
	function goDeleteSlip() {
		if(formValidate("#main")) {
			if(confirm('전표를 삭제하시겠습니까?')) {
				removeMask("#main");				
				document.main.action = 'wel12001_delete_slip.do';
				document.main.target = '_self';
				document.main.submit();
			}
		}
	...
	%>	
		...
		document.form1.pay_amt_tot.value = parseInt2(document.form1.member_cnt) * parseInt2(document.form1.pay_amt);
...
<input type="hidden" name="excel_file_nm" 	value="워크샵 일괄지급">
<% if(!box.get("msg_popup").equals("Y")) { %>
	<h1 class="page-title">워크샵 일괄지급</h1>
<% } %>
		...
		<% if(!box.get("bugt_cd").equals("")) { %>
	  		...
	  		<tr>
				...
				<td colspan="6" class="t-left">
						<input type="text" name="etc_remark" value="<%=obox.get("etc_remark")%>" maxlength="50" title="적요" class="bs-select form-control" style="width:300px;ime-mode:active;">
				</td>
	  		</tr>
		<% } %>
			...
			<div class="form-inline f-right">
				<div class="form-group">
			  	<% if(!obox.get("slip_no").equals("")) { %>
					...
					= <span style="font-weight:bold; color:#01a1db;"><%=FormatUtil.insertComma(String.valueOf(obox.getInt("pay_amt") ))%></span> 원
			  	<% }else if(isEditable) { %>
			  		<b>총 지급액</b> : 
		...
		<div class="modal-dialog" style="width:400px;">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">워크샾 대상자 추출 조건설정</h4>
				</div>
...
<input type="hidden" name="subject" 		value="워크샾(<%=obox.get("bugt_nm")%>)">
<input type="hidden" name="msg_type" 		value="SLP012">
