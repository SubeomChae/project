...
<title>건강증진활동지원</title>
</head>
<body>
<%	String apply_no = box.get("params");
	Box obox = CodeHelper.getRsBox("PR_REPORT_SQL", "WEL01001", apply_no, "S018","","","",request);
	...
	Paragraph title = new Paragraph("건강증진활동지원", font_title);
	...
	table.addCell(PdfUtil.cellFormat("체육시설", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("data01"), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
	table.addCell(PdfUtil.cellFormat("연락처", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("data03"), font_data, "LEFT", "", "", "m25", "0,5,0,5"));

	table.addCell(PdfUtil.cellFormat("등록금액", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("data08",Box.THOUSAND_COMMA), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
	table.addCell(PdfUtil.cellFormat("기간", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("data05",Box.DEF_DATE_FMT) + "~" + obox.get("data06",Box.DEF_DATE_FMT)+" (일수:"+obox.get("data07")+"일)", font_data, "LEFT", "", "", "m25", "0,5,0,5"));
