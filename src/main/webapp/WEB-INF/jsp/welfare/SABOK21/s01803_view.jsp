...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("문화활동비지원 신청서 조회");	
		<% } %>
	});
</script>
<form id="viewForm" name="main" method="post" action="" class="fType" enctype="multipart/form-data" style="margin:0px">
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		...
		<tr>
			<th>일자</th>
			<td colspan="3"><%=obox.get("data01",Box.DEF_DATE_FMT)%></td>
		</tr>
		<tr height="50">
			<th>행사내용</th>
			<td colspan="3"><%=obox.get("data04",Box.MULTILINE_TEXT)%></td>
		</tr>
		<tr>
			<th>신청일자</th>
