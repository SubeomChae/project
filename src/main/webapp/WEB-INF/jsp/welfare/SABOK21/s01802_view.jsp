...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("건강증진활동 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>체육시설</th>
			<td width="35%"><%=obox.get("data01")%></td>
			<th>연락처</th>
			<td width="35%"><%=obox.get("data03")%></td>
		</tr>
		<tr>
			<th>등록금액</th>
			<td colspan="3"><%=obox.get("data08",Box.THOUSAND_COMMA)%></td>
		</tr>
		<tr>
			<th>기간</th>
			<td colspan="3"><%=obox.get("data05",Box.DEF_DATE_FMT)%><%if(!obox.get("data06").equals(obox.get("data05"))){%> ~ <%=obox.get("data06",Box.DEF_DATE_FMT)%><%}%>&nbsp;&nbsp;(일수 : <%=obox.get("data07")%> 일)</td>
		</tr>
		<tr>
			<th>신청일자</th>
...
<input type="hidden" name="subject" 		value="건강증진활동 신청(<%=obox.get("empl_nm")%>)">
