...
<%	RsWrapper codeWp = CodeUtil.getRsWrapper("PR_COMTB_CODE02_LIST","MULTCODE", "", "tel_no|bank_cd@@@@A|","","","","","");
...
<%@ include file="/WEB-INF/jsp/welfare/payGuide.jsp" %>
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th width="15%">체육시설명</th>
			<td width="35%">
				<input type="text" name="course_nm" value="<%=obox.get("data01")%>" size="30" maxlength="50" title="교육명" class="form-control required" style="width:150px">	
			</td>
			<th width="15%">연락처</th>
			<td width="35%">
				<select name="edu_org_tel1" title="전화번호" class="bs-select form-control" style="width:60px">
					<option value=''>선택</option>
					<%=CodeUtil.optList(codeWp, "tel_no", StringUtil.arrayCol(obox.get("data03"),1,"-"))%>
				</select> - 
				<input type="text" name="edu_org_tel2" value="<%=StringUtil.arrayCol(obox.get("data03"),2,"-")%>" size="4" maxlength="4" title="전화번호" class="form-control autoTab" style="width:60px"> - 
				<input type="text" name="edu_org_tel3" value="<%=StringUtil.arrayCol(obox.get("data03"),3,"-")%>" size="4" maxlength="4" title="전화번호" class="form-control autoTab" style="width:60px">
			</td>
		</tr>
		<tr>
			<th>등록금액</th>
			<td colspan="3">
				<input type="text" name="use_amt" value="<%=obox.get("data08")%>" size="13" maxlength="10" title="교육비" class="form-control required tMONEY" style="width:90px;" > 원
			</td>
		</tr>
		<tr>
			<th>기간</th>
			<td colspan="3">
				<div class="form-inline">
					<div class="input-group input-medium date date-picker" data-date="<%=obox.get("data05",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" name="start_ymd" value="<%=obox.get("data05")%>" maxlength="8" title="일자(부터)" class="form-control tYMD" onchange="calendarSettingPost('start_ymd')">
						<span class="input-group-btn"><button class="btn dark" type="button"><i class="fa fa-calendar"></i></button></span>
					</div>
					<span>~</span>
					<div class="input-group input-medium date date-picker" data-date="<%=obox.get("data06",Box.DEF_DATE_FMT)%>" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" name="end_ymd" value="<%=obox.get("data06")%>" maxlength="8" title="일자(까지)" class="form-control tYMD" onchange="calendarSettingPost('end_ymd')">
						<span class="input-group-btn"><button class="btn dark" type="button"><i class="fa fa-calendar"></i></button></span>
					</div>
					(일수 : <input type="text" name="edu_ilsu" value="<%=obox.get("data07")%>" maxlength="3" title="일수" class="form-control tMONEY" style="width:25px" readOnly>&nbsp;일)
				</div>
			</td>
		</tr>
		<tr>
