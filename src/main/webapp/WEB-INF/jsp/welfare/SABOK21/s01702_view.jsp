...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("상해보험지원 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th width="15%">구분</th>
			<td width="35%"><%=obox.get("sub_nm")%></td>
			<th width="15%">신청일자</th>
			<td width="35%"><%=obox.get("apply_ymd",Box.DEF_DATE_FMT)%></td>
		</tr>
		<tr>
			<th>보험료</th>
			<td><%=obox.get("data01",Box.THOUSAND_COMMA)%> 원</td>
			<th>보험기간</th>
			<td colspan="3"><%=obox.get("data02",Box.DEF_DATE_FMT)%> ~ <%=obox.get("data03",Box.DEF_DATE_FMT)%></td>
		</tr>
		<tr>
			<th>신청금액</th>
			<td><%=obox.get("apply_amt",Box.THOUSAND_COMMA)%> 원</td>
			<th>보험수익자</th>
			<td><%=obox.get("data04",Box.MULTILINE_TEXT)%></td>
		</tr>
		<tr>
			<th>비고</th>
