	...
	function goSave() {
		if (formValidate("#saveForm")) {
			...
			if(confirm('저장하시겠습니까?')) {
				...
				document.saveForm.data09.value = document.saveForm.multi_child_yn.value;
				document.saveForm.data10.value = document.saveForm.free_supply_yn.value;
				document.saveForm.data14.value = document.saveForm.license_yn.value;
	...
	<div class="table-scrollable">
		<table class="table table-view table-bordered">
			...
			<tr>
				<th width="20%">학교명</th>
				<td width="30%">
					<input type="text" name="school_nm" value="<%=obox.get("school_nm")%>" maxlength="30" title="학교명" class="form-control required" style="width:90px">
				...
				<td width="30%">
					<div class="input-group input-medium date date-picker" data-date="" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
						<input type="text" name="major_nm" value="<%=box.get("apply_no").equals("") ? DateUtil.toString("yyyyMMdd") : obox.get("major_nm")%>" maxlength="8" title="신청일자" class="form-control required tYMD" <%=isEditable?"":"readonly"%>>
			...
			<tr>
				<th>다자녀 여부</th>
				<td>
					<select name="multi_child_yn" title="다자녀여부" class="bs-select form-control" style="width:117px">
						<option value='N' <%=!obox.get("multi_child_yn").equals("Y")?"selected":""%>>아니오</option>
						<option value='Y' <%=obox.get("multi_child_yn").equals("Y")?"selected":""%>>예</option>
			        </select>
				</td>
				<th>무상지원 여부</th>
				<td>
					<select name="free_supply_yn" title="무상지원여부" class="bs-select form-control" style="width:117px">
						<option value='N' <%=!obox.get("free_supply_yn").equals("Y")?"selected":""%>>아니오</option>
						<option value='Y' <%=obox.get("free_supply_yn").equals("Y")?"selected":""%>>예</option>
			        </select>
				</td>
			</tr>
			<tr>
				<th>비인가 학교 여부</th>
				<td colspan="3">
					<select name="license_yn" title="비인가학교여부" class="bs-select form-control" style="width:117px">
						<option value='N' <%=!obox.get("license_yn").equals("Y")?"selected":""%>>아니오</option>
						<option value='Y' <%=obox.get("license_yn").equals("Y")?"selected":""%>>예</option>
			        </select>
				</td>
			</tr>
			<tr>
