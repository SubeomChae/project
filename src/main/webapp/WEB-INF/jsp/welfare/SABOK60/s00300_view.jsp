...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<div class="table-scrollable">
		<table class="table table-view table-bordered">
			...
			<tr>
				<th width="20%">학교명</th>
				<td width="30%"><%=obox.get("school_nm")%><%=obox.get("school_flag_nm")%></td>	
				<th width="20%">입학일자</th>
				<td width="30%"><%=obox.get("major_nm",Box.DEF_DATE_FMT)%></td>				
			</tr>
			<tr>
				<th>다자녀 여부</th>
				<td><%=obox.get("multi_child_yn").equals("Y")?"예":"아니오"%></td>
				<th>무상지원 여부</th>
				<td><%=obox.get("free_supply_yn").equals("Y")?"예":"아니오"%></td>
			</tr>
			<tr>
				<th>비인가 학교 여부</th>
				<td colspan="3"><%=obox.get("license_yn").equals("Y")?"예":"아니오"%></td>
			</tr>
			<tr>
				<th>신청일자</th>
