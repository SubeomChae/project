...
<input type="hidden" name="etc_remark" 		value="">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th width="20%">이자지원 종류</th>
			<td width="30%"><%=obox.get("support_item_nm")%></td>
			<th width="20%">대출시행(예정)일</th>
			<td width="30%"><%=obox.get("loan_ymd",Box.DEF_DATE_FMT)%></td>
		</tr>
		<tr>
			<th>은행</th>
			<td><%=obox.get("loan_bank_nm")%></td>
			<th>대출금액</th>
			<td><%=obox.get("support_amt",Box.THOUSAND_COMMA)%> 원</td>
		</tr>
		<tr>
			<th>전용면적</th>
			<td><%=obox.get("use_area",Box.THOUSAND_COMMA_FLOAT)%> ㎡</td>
			<th>주택종류</th>
			<td><%=obox.get("house_kind")%></td>
		</tr>
		<tr>
			<th>매입·임차계약일</th>
			<td><%=obox.get("contract_ymd",Box.DEF_DATE_FMT)%></td>
			<th>취득완료일</th>
			<td><%=obox.get("obtain_ymd",Box.DEF_DATE_FMT)%></td>
		</tr>
		<tr>
			<th>주소</th>
			<td colspan="3"><%if(!box.get("new_zip_no").equals("")){%>[<%=obox.get("new_zip_no",Box.ZIP_CODE)%>]<%}%> <%=obox.get("new_addr")%></td>
		</tr>
		<tr>
			<th>신청일자</th>
