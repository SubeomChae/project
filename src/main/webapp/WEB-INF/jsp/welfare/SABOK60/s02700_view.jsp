...
<input type="hidden" name="msg_popup" 	value="<%=box.get("msg_popup")%>">
	<table class="table table-view table-bordered table-cover marB5">
		...
		<tr>
			<th width="20%">주민번호</th>
			<td width="30%"><%=FormatUtil.juminNo(obox.get("reside_no"),"x")%></td>
			<th width="20%">직위</th>
			<td width="30%"><%=obox.get("position_nm")%></td>
		</tr>
		<tr>
			<th>입사일자</th>
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>대상자</th>
			<td><%=obox.get("family_rel_nm").equals("")?"":"["+obox.get("family_rel_nm")+"]"%> <%=obox.get("family_nm")%></td>
			<th>장애정도</th>
			<td><%=obox.get("handicap_level_nm")%></td>
		</tr>
		<tr>
			<th width="20%">신청일자</th>
