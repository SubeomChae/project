...
<title>휴양시설이용신청서</title>
</head>
<body>
<%	String apply_no = box.get("params");
	Box obox = CodeHelper.getRsBox("PR_REPORT_SQL", "WEL01001", apply_no, "S005","","","",request);
	...
	Paragraph title = new Paragraph("휴양시설이용신청서", font_title);
	...
	table.addCell(PdfUtil.cellFormat("휴양시설명", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("condo_nm"), font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
	table.addCell(PdfUtil.cellFormat("이용일자", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("start_ymd",Box.DEF_DATETIME_FMT)+" ~ "+obox.get("end_ymd",Box.DEF_DATETIME_FMT)+" (이용일수 : "+obox.get("use_ilsu")+"일)", font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
	table.addCell(PdfUtil.cellFormat("사용인원", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("member_cnt")+"명", font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
	table.addCell(PdfUtil.cellFormat("이용금액", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("use_amt",Box.THOUSAND_COMMA), font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
