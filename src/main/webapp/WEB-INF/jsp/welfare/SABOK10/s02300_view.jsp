...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("꽃바구니 지원 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			<th>구입일자</th>
			<td colspan="3"><%=obox.get("start_ymd",Box.DEF_DATE_FMT)%></td>
		</tr>
		<tr>
			<th width="15%">신청일자</th>
	...
	<div class="table-bottom-control">
		...
		<% } %>
			<button type="button" class="btn dark" onClick="goUbiViewer('WEB-INF/jsp/welfare/<%=box.get("dbo")%>/report/s023.jrf', 'P_PARAM01!SLP002,<%=box.get("apply_no")%>!P_PARAM02!ACCTB_SLIP01|<%=obox.get("slip_no")%>|<%=obox.get("apply_no")%>|<%=obox.get("bugt_cd")%>,<%=obox.get("bugt_cd")%>!');"><img src="/com/img/ico_btn_print.png">인쇄</button>
		</span>
	</div>
</form>
...
<input type="hidden" name="subject" 		value="꽃바구니 신청(<%=obox.get("empl_nm")%>)">
