...
<script language="javascript">
	...
	function page_init() {
		if(!isEmpty(document.saveForm.condo_cd1)) { change_condo_cd('<%=obox.get("condo_cd1")%>', '<%=obox.get("condo_cd2")%>'); }
	}
</script>
...
<%@ include file="/WEB-INF/jsp/welfare/payGuide.jsp" %>
	...
	<table class="table table-view table-bordered table-cover marB5">
		...
		<tr>
			<th>사용인원</th>
			<td colspan="3">
				<input type="text" name="member_cnt" value="<%=obox.get("member_cnt")%>" size="3" maxlength="40" title="사용인원" class="form-control" style="width:55px"> 명
			</td>
		</tr>
		<tr>
			<th>이용금액</th>
			<td colspan="3">
				<input type="text" name="use_amt" value="<%=obox.get("use_amt")%>" size="13" maxlength="10" title="신청금액" class="form-control required tMONEY" style="width:100px"> 원
			</td>
		</tr>
		<tr>
			<th width="15%">신청일자</th>
