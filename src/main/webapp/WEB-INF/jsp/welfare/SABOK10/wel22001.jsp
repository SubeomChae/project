...
<% } %>
	...
	<% RsWrapper codeWp = CodeUtil.getRsWrapper("PR_BUDTB_CODE01_LIST","LIST001", "", "S","","","N","",uSesEnt.dbo); 
	   int column_cnt = codeWp.getLength() + 4; int col_width = 110 / column_cnt; %>	
			...
			<% }else {
					...
					<tr > 
			   	   <% } %>
						...
						<td class="t-center"><%if(!box.get("excel_yn").equals("Y") && !rsWp.get("empl_nm",i).equals("합계")){%><a href="javascript:goEmployeePop('<%=rsWp.get("empl_no",i)%>')"><%}%><%=rsWp.get("empl_nm",i)%></a></td>
						<td class="t-center" style="letter-spacing:-1px"><%=rsWp.get("dept_nm",i)%></td>
					<% for(int k=0; k < codeWp.getLength(); k++) { %>	
