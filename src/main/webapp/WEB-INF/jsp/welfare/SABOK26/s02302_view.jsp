...
<script language="javascript">
	...
	$(function() {
		<% if(box.get("popup_yn").equals("Y")) { %>
			$(".page-logo-text").html("출산축하용품 신청서 조회");	
		<% } %>
	});
</script>
...
<input type="hidden" name="apply_no" 	value="<%=box.get("apply_no")%>">
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			...
			<td width="35%"><%=obox.get("family_birth_ymd",Box.DEF_DATE_FMT)%></td>
		</tr>
		<tr>
			<th>신청일자</th>
...
<input type="hidden" name="subject" 		value="기념품지급 신청(<%=obox.get("empl_nm")%>)">
