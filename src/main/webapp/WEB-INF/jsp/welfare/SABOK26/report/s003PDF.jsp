...
<%	String apply_no = box.get("params");
	...
	Paragraph title = new Paragraph("학자금신청서", font_title);
	...
	if(obox.get("school_term").equals("")) {
		table.addCell(PdfUtil.cellFormat(obox.get("school_nm")+" "+obox.get("school_flag_nm")+" "+obox.get("school_year")+"학년 ", font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
	}else {
		table.addCell(PdfUtil.cellFormat(obox.get("school_nm")+" "+obox.get("school_flag_nm")+" "+obox.get("school_year")+"학년 "+obox.get("school_term")+"학기", font_data, "LEFT", "", "", "m25", "0,5,0,5", "col3"));
	}
	table.addCell(PdfUtil.cellFormat("학과", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("major_nm"), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
	...
	table.addCell(PdfUtil.cellFormat("학점", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("grade_score"), font_data, "LEFT", "", "", "m25", "0,5,0,5"));
	table.addCell(PdfUtil.cellFormat("납부금액", font_head, "", "", "LIGHT_GRAY", "m25", ""));
	table.addCell(PdfUtil.cellFormat(obox.get("pay_amt",Box.THOUSAND_COMMA)+"원", font_data, "LEFT", "", "", "m25", "0,5,0,5"));
