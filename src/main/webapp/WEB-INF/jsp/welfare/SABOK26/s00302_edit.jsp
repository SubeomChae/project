...
<%@ include file="/WEB-INF/jsp/welfare/payGuide.jsp" %>
	...
	<table class="table table-view table-bordered table-cover marB5">
		<tr>
			...
			<% }else { %>
				<select name="school_flag" title="학교" class="form-control required" style="width:100px">
		...
		<tr>	
			<th width="15%">학과</th>
			<td width="35%">
				<input type="text" name="major_nm" value="<%=obox.get("major_nm")%>" size="20" maxlength="20" class="form-control" title="학과" style="width:140px">
			</td>
			<th width="15%">대상자</th>
			<td width="35%">
			<% if(uSesEnt.dbo.equals("SABOK26")) { %>
			...
			<% }else { %>
				...
				<%	RsWrapper familyWp = CodeUtil.getRsWrapper("PR_INSTB_EMPL12_LIST","LISTCODE", "", obox.get("empl_no"),"00","","","","");
	...
	<% if(uSesEnt.dbo.equals("SABOK26")) { %>
		<tr>
			<th>학점</th>
			<td>
				<input type="text" name="grade_score" value="<%=obox.get("grade_score")%>" size="13" maxlength="10" class="form-control required" title="학점" style="width:90px;" >
			</td>
			<th>납부금액</th>
			<td>
				<input type="text" name="pay_amt" value="<%=obox.get("pay_amt")%>" size="13" maxlength="10" class="form-control required tMONEY" title="납부금액" style="width:90px;" > 원
			</td>
		</tr>
	<% } %>
		<tr>
