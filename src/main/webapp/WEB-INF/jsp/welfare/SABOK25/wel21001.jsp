...
<script language='javascript'>
	...
	function change_bugt_cd(bugt_cd) {
		...
		if(bugt_cd == 'S001') {
			...
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='required' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeHelper.OptListJs("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S008","","","", request)%></select> ";
			document.main.excel_file_nm.value = '연금지급현황';
		}else if(bugt_cd == 'S022') {
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='required' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeHelper.OptListJs("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S022","","","", request)%></select> ";
			document.main.excel_file_nm.value = '의료비 지원현황';
		}else if(bugt_cd == 'S023') {
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='required' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeHelper.OptListJs("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S023","","","", request)%></select> ";
			document.main.excel_file_nm.value = '의료비 지원현황';
		}else if(bugt_cd == 'S024') {
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='required' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeHelper.OptListJs("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S024","","","", request)%></select> ";
			document.main.excel_file_nm.value = '학자금 지원현황';
		}else if(bugt_cd == 'S025') {
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='required' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeHelper.OptListJs("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S025","","","", request)%></select> ";
			document.main.excel_file_nm.value = '학자금 지원현황';
		}else if(bugt_cd == 'S026') {
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='required' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeHelper.OptListJs("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S026","","","", request)%></select> ";
			document.main.excel_file_nm.value = '장기근속 지원현황';
		}else if(bugt_cd == 'S027') {
			cHtml = "<b>세부사업명</b> : "
				  + "<select name='detail_key' class='required' style='width:170px'>"
				  + "<option value=''>전체</option><%=CodeHelper.OptListJs("PR_BUDTB_CODE01_LIST","LISTCODE","","2","S027","","","", request)%></select> ";
			document.main.excel_file_nm.value = '장기근속 지원현황';
		}else if(bugt_cd == '') {
			cHtml = "";
