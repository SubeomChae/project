...
<%@ page import="java.io.*, java.util.*, egovframework.cbiz.*, egovframework.cbiz.util.*, egovframework.ubiz.util.*, com.innorix.transfer.InnorixUpload" %>
<%
	// CORS체크를 위한 OPTIONS 메소드가 들어오므로 POST 일 경우에만 동작하도록 조건 추가
	if(request.getMethod() == "POST") {
		String destDir = "";
		if(request.getParameter("biz_id").equals("ontong")) { // 설계도서
			destDir = OntongUtil.getStorageDir("T") + "T/temp/" + request.getParameter("upload_key");
		}else if(request.getParameter("biz_id").equals("eReport")) { // e보고서
			destDir = OntongUtil.getStorageDir("E") + "temp/" + request.getParameter("upload_key");
		}
		if(!FileUtil.isExistFile(destDir)) { FileUtil.mkDirs(destDir); }
		int maxPostSize = 2147482624; // bytes
		
		InnorixUpload uploader = new InnorixUpload(request, response, maxPostSize, destDir);
		...
		************************************************/
		...
		if(uploader.isUploadDone()) {
			System.out.println(System.currentTimeMillis() + " ["+_run_retval+"] " + destDir+"/"+_orig_filename + " : " + _filesize + " byte");
