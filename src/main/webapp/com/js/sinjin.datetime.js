...
function isValidYmd(params) {
	var form_nm = 'main'; var input_nm = params;
	if(params.indexOf('.') > 0) {
		arr = params.split('.');
		form_nm = arr[0]; input_nm = arr[1];
	}
	
	var yyObject = eval('document.' + form_nm + '.' + input_nm + '1');
	var mmObject = eval('document.' + form_nm + '.' + input_nm + '2');
	var ddObject = eval('document.' + form_nm + '.' + input_nm + '3');
	if(!isEmpty(yyObject) || !isEmpty(mmObject) || !isEmpty(ddObject)) {
		if(!yyObject.value.isNum() || !mmObject.value.isNum() || !ddObject.value.isNum()) {
	        return false;
...
function sysdateAMPM() {
    ...
    var hour  = date.getHours();
    var min   = date.getMinutes();
    var dateFmt = year + '-' + (month+'').padLeft(2,'0') + '-' + (day+'').padLeft(2,'0');
	if(hour >= 0 && hour <= 12) {
		dateFmt += ' 오전 ' + (hour+'').padLeft(2,'0') + ':' + (min+'').padLeft(2,'0');
	}else {
		hour = hour - 12;
		dateFmt += ' 오후 ' + (hour+'').padLeft(2,'0') + ':' + (min+'').padLeft(2,'0');
	}
    return dateFmt;
}
