...
function Folder(folderDescription, hreference) { 
	...
	this.isOpen = true;
	this.iconSrc = "/com/img/tree/ftv2folderopen.gif";
	...
	if(folder.isOpen) { 
		if(folder.nodeImg) {
			if(folder.isLastNode) {
				folder.nodeImg.src = "/com/img/tree/ftv2mlastnode.gif";
			}else {
				folder.nodeImg.src = "/com/img/tree/ftv2mnode.gif";
			}
		}
		folder.iconImg.src = "/com/img/tree/ftv2folderopen.gif" 
		for(i=0; i<folder.nChildren; i++) { folder.children[i].display(); }
	}else { 
		if(folder.nodeImg) {
			if(folder.isLastNode) {
				folder.nodeImg.src = "/com/img/tree/ftv2plastnode.gif";
			}else {
				folder.nodeImg.src = "/com/img/tree/ftv2pnode.gif";
			}
		}
		folder.iconImg.src = "/com/img/tree/ftv2folderclosed.gif" 
...
function initializeFolder(level, lastNode, leftSide) {
	...
	if(level > 0) {
		if(lastNode) {//the last 'brother' in the children array 
			this.renderOb(leftSide + auxEv + "<img name='nodeIcon" + this.id + "' src='/com/img/tree/ftv2mlastnode.gif' border=0></a>");
			leftSide = leftSide + "<img src='/com/img/tree/ftv2blank.gif'>";
			this.isLastNode = 1;
		}else { 
			this.renderOb(leftSide + auxEv + "<img name='nodeIcon" + this.id + "' src='/com/img/tree/ftv2mnode.gif' border=0></a>");
			leftSide = leftSide + "<img src='/com/img/tree/ftv2vertline.gif'>";
			this.isLastNode = 0;
		}
	}else {
		this.renderOb("");
...
function Item(itemDescription, itemLink) { 
	...
	this.navObj = 0;
	this.iconImg = 0;
	this.iconSrc = "/com/img/tree/ftv2doc.gif";
...
function Item2(itemDescription, itemLink) { 
	this.desc = itemDescription;
	this.link = itemLink;
	this.id = -1;
	this.navObj = 0;
	this.iconImg = 0;
	this.iconSrc = "/com/img/tree/ftv2link.gif";
...
function initializeItem(level, lastNode, leftSide) {	
	...
	if(level > 0) {
		if(lastNode) { 
			this.renderOb(leftSide + "<img src='/com/img/tree/ftv2lastnode.gif'>");
			leftSide = leftSide + "<img src='/com/img/tree/ftv2blank.gif'>";
		}else { 
			this.renderOb(leftSide + "<img src='/com/img/tree/ftv2node.gif'>");
			leftSide = leftSide + "<img src='/com/img/tree/ftv2vertline.gif'>";
		}
	}else {
		this.renderOb("");
...
function drawItem(leftSide) { 
	...
	var menu_id = "";
	if(location.href.indexOf("staMain.do") > 0) { menu_id = this.link.replace("javascript:parent.goMenu(","").substring(2,5); }
	if(menu_id != '') { doc.write(" menu_id='"+menu_id+"' menu_node='"+this.id+"' "); }
	...
	doc.write("</a>");
	doc.write("</td><td id=tr_"+this.id+" valign='middle' nowrap>");
	if(USETEXTLINKS) {
...
function gLnk(target, description, linkData, itemImg) { 
	...
	var pos = tooltip.indexOf('<font');
	if(pos >= 0) {
 		pos = tooltip.indexOf('>',pos);
 		tooltip = tooltip.substring(pos+1);
 		pos = tooltip.indexOf('</font>',pos);
 		tooltip = tooltip.substring(0,pos);
	}
 
	if(target == 0)	{ 
		if(linkData != "") {
			fullLink = "'"+linkData+"' title=\"" +tooltip+ "\" target=\"_self\"";
		}else {
			fullLink = "";
		}
	}else if(target == 1) {
		if(linkData != "") {
			fullLink = "'http://"+linkData+"' title=\"" +tooltip+ "\" target=_self";
		}else {
			fullLink = "";
		}
	}else { 
