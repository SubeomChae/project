  ...
  // @return : String
  //-----------------------------------------------------------------------------
  String.prototype.fmtFileSize = function() {
		var fileSize = parseInt(this);
		var sizeUnit = 'Byte';
		if(fileSize > 1024) { fileSize = fileSize / 1024; sizeUnit = 'KB'; }
		if(fileSize > 1024) { fileSize = fileSize / 1024; sizeUnit = 'MB'; }
		if(fileSize > 1024) { fileSize = fileSize / 1024; sizeUnit = 'GB'; }
		
		return fileSize.toFixed(2) + " " + sizeUnit;
  }
  ...
  String.prototype.fmtTextArea = function() {
      var val = this.replace(/<br>/g,"\n");
      return val;
  }
  ...
  String.prototype.padRight = function(cnt, padStr) {
      var digit = "";
      if (this.length < cnt) {
          for(var i=0; i < cnt - this.length; i++) {
              digit += padStr;
...
//-----------------------------------------------------------------------------
  ...
  String.prototype.nvl = function() {
      ...
      var str = this.trim();
      if(this.isBlank() || this == undefined) return arg;
      else return this;
  }
  ...
  String.prototype.isNumCol = function() {
      return (/^[0-9;]+$/).test(this.remove(arguments[0])) ? true : false;
  }
  ...
  String.prototype.isFmsUserid = function() {
	  if(this.length < 4) {
		  alert("아이디는 4자리 이상이어야 합니다.");
		  return false;
	  }else if(!this.isEngNum()) {
  ...
  String.prototype.isKor = function() {
      ...
      var numUnicode = this.charCodeAt(0);
  ...
  // @return : 한글화된 숫자
  //-----------------------------------------------------------------------------
  String.prototype.moneyToKOR = function(unit) {
    var chknum = this;
    num = chknum;
    num = num.split(',').join('');
    var arr = num.split('.');
    ...
    if(unit > 0) { num = (num * parseInt(unit)) + ""; }
    if (!re.test(num)) {
        chknum.value = '';
        return '';
    } else {
  ...
  //-----------------------------------------------------------------------------
  // 문자열 배열에 특정 문자열이 포함되는지 검사
  // @return : boolean
  //-----------------------------------------------------------------------------
  String.prototype.containsElement = function(target, dmt) {
      var srcArr = this.split(dmt);
  ...
  //-----------------------------------------------------------------------------
  // 문자열 배열에서 특정 인덱스 문자열을 추출
  // @return : String
  //-----------------------------------------------------------------------------
  String.prototype.arrayCol = function(pos, dmt) {
	  if(this == "") { return ""; }
	  if(dmt == undefined || dmt == "") { dmt = "|"; }
      var srcArr = this.split(dmt);
      if(pos > srcArr.length) { return ""; }
      return srcArr[pos] == null ? "" : srcArr[pos];
  }
